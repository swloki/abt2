...
const INVOICE_COLUMNS: &str = "id, doc_number, customer_id, issue_date, period, subtotal, tax_amount, total, status, source_shipping_id, gl_entry_id, due_date, outstanding_amount, paid_amount, is_return, return_against, operator_id, version, created_at, updated_at, deleted_at";
...
impl SalesInvoiceRepo {
    ...
    /// 创建红字发票记录（is_return=true, return_against=original_id）
    pub async fn create_red(
        ...
        let id: i64 = sqlx::query_scalar::<sqlx::Postgres, i64>(
            r#"INSERT INTO sales_invoices
               (doc_number, customer_id, issue_date, period, subtotal, tax_amount, total, status,
                source_shipping_id, is_return, return_against, operator_id)
        ...
        .bind(original.total)
        .bind(InvoiceStatus::Draft)
        .bind(original.source_shipping_id)
        .bind(original.id)
        .bind(operator_id)
        .fetch_one(executor)
        .await?;
