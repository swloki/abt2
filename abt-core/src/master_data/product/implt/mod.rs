...
impl ProductService for ProductServiceImpl {
    async fn create(&self, mut ctx: ServiceContext<'_>, req: CreateProductReq) -> Result<i64, DomainError> {
        ...
        self.audit.record(ctx, "Product", id, AuditAction::Create, None, None).await?;
    ...
    async fn update(&self, mut ctx: ServiceContext<'_>, id: i64, req: UpdateProductReq) -> Result<(), DomainError> {
        ...
        if let Some(new_status) = req.status {
            if new_status != existing.status {
                ...
                self.event_bus.publish(
                    ctx.reborrow(),
                    EventPublishRequest {
                        event_type: DomainEventType::ProductStatusChanged,
        ...
        self.audit.record(ctx, "Product", id, AuditAction::Update, None, None).await?;
        Ok(())
    }
    ...
    async fn check_product_usage(&self, ctx: ServiceContext<'_>, _id: i64, query: UsageQuery) -> Result<PaginatedResult<UsageEntry>, DomainError> {
