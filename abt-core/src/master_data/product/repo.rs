...
impl ProductRepo {
    pub async fn create(
        ...
        let row = sqlx::query_scalar::<sqlx::Postgres, i64>(
            r#"INSERT INTO products (pdt_name, product_code, unit, status, acquire_channel, external_code, owner_department_id, meta)
        ...
        .bind(req.acquire_channel.as_i16())
    ...
    pub async fn update(
        ...
        if req.acquire_channel.is_some() { sets.push(format!("acquire_channel = ${param_idx}")); param_idx += 1; }
        ...
        if let Some(v) = req.acquire_channel { q = q.bind(v.as_i16()); }
