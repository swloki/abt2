...
impl SupplierRepo {
    pub async fn create(
        ...
        let row = sqlx::query_scalar::<sqlx::Postgres, i64>(
            r#"INSERT INTO suppliers (supplier_code, supplier_name, category, status, tax_number, remark, operator_id)
    ...
    pub async fn find_by_id(&self, executor: PgExecutor<'_>, id: i64) -> Result<Option<Supplier>> {
        let supplier = sqlx::query_as::<sqlx::Postgres, Supplier>(
            "SELECT supplier_id, supplier_code, supplier_name, category, status, tax_number, remark, operator_id, created_at, updated_at, deleted_at \
    ...
    pub async fn query(
        ...
        let data_sql = format!(
            "SELECT supplier_id, supplier_code, supplier_name, category, status, tax_number, remark, operator_id, created_at, updated_at, deleted_at \
             FROM suppliers WHERE {where_clause} ORDER BY supplier_id DESC LIMIT ${limit_idx} OFFSET ${offset_idx}",
        );
    ...
    /// Returns true if the tax_number already exists.
...
impl SupplierContactRepo {
    ...
    pub async fn find_by_id(&self, executor: PgExecutor<'_>, contact_id: i64) -> Result<Option<SupplierContact>> {
        let contact = sqlx::query_as::<sqlx::Postgres, SupplierContact>(
            "SELECT contact_id, supplier_id, contact_name, phone, email, position, is_primary, created_at, updated_at \
             FROM supplier_contacts WHERE contact_id = $1",
        )
        .bind(contact_id)
        .fetch_optional(executor)
        .await?;
    ...
    pub async fn find_by_supplier_id(&self, executor: PgExecutor<'_>, supplier_id: i64) -> Result<Vec<SupplierContact>> {
        let contacts = sqlx::query_as::<sqlx::Postgres, SupplierContact>(
            "SELECT contact_id, supplier_id, contact_name, phone, email, position, is_primary, created_at, updated_at \
             FROM supplier_contacts WHERE supplier_id = $1 ORDER BY is_primary DESC, contact_id ASC",
        )
        .bind(supplier_id)
        .fetch_all(executor)
        .await?;
...
impl SupplierBankAccountRepo {
    pub async fn create(
        ...
        let row = sqlx::query_scalar::<sqlx::Postgres, i64>(
            r#"INSERT INTO supplier_bank_accounts (supplier_id, bank_name, account_name, account_number, branch, is_default)
               VALUES ($1, $2, $3, $4, $5, $6)
    ...
    pub async fn update(
        ...
        let before = sqlx::query_as::<sqlx::Postgres, SupplierBankAccount>(
            "SELECT account_id, supplier_id, bank_name, account_name, account_number, branch, is_default, created_at, updated_at \
             FROM supplier_bank_accounts WHERE account_id = $1 AND supplier_id = $2",
        )
        .bind(account_id)
        .bind(supplier_id)
        ...
        if req.branch.is_some() { sets.push(format!("branch = ${param_idx}")); param_idx += 1; }
        ...
        if let Some(ref v) = req.branch { q = q.bind(v); }
    ...
    pub async fn find_by_id(&self, executor: PgExecutor<'_>, account_id: i64) -> Result<Option<SupplierBankAccount>> {
        let account = sqlx::query_as::<sqlx::Postgres, SupplierBankAccount>(
            "SELECT account_id, supplier_id, bank_name, account_name, account_number, branch, is_default, created_at, updated_at \
             FROM supplier_bank_accounts WHERE account_id = $1",
        )
        .bind(account_id)
        .fetch_optional(executor)
        .await?;
    ...
    pub async fn find_by_supplier_id(&self, executor: PgExecutor<'_>, supplier_id: i64) -> Result<Vec<SupplierBankAccount>> {
        let accounts = sqlx::query_as::<sqlx::Postgres, SupplierBankAccount>(
            "SELECT account_id, supplier_id, bank_name, account_name, account_number, branch, is_default, created_at, updated_at \
             FROM supplier_bank_accounts WHERE supplier_id = $1 ORDER BY is_default DESC, account_id ASC",
        )
        .bind(supplier_id)
        .fetch_all(executor)
        .await?;
