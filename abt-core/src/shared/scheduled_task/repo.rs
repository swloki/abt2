...
impl ScheduledTaskRepo {
    pub async fn find_by_name(&self, executor: PgExecutor<'_>, name: &str) -> RepoResult<Option<ScheduledTaskDef>> {
    ...
    pub async fn try_acquire_lock(&self, executor: PgExecutor<'_>, task_id: i64) -> RepoResult<bool> {
    ...
    pub async fn release_lock(&self, _executor: PgExecutor<'_>, _task_id: i64) -> RepoResult<()> {
        // 事务级锁在事务结束时自动释放，无需手动 unlock
        Ok(())
    }

    pub async fn update_last_run(&self, executor: PgExecutor<'_>, task_id: i64, elapsed_ms: i64, result: Option<&str>, error: Option<&str>) -> RepoResult<()> {
        sqlx::query(
    ...
    pub async fn insert_run_log(&self, executor: PgExecutor<'_>, task_id: i64, status: i16, elapsed_ms: Option<i64>, result: Option<&str>, error: Option<&str>) -> RepoResult<()> {
        sqlx::query(
    ...
    pub async fn list_tasks(&self, executor: PgExecutor<'_>) -> RepoResult<Vec<ScheduledTaskDef>> {
    ...
    pub async fn list_run_logs(&self, executor: PgExecutor<'_>, task_id: i64, limit: i64) -> RepoResult<Vec<TaskRunLog>> {
