...
impl ActionRegistry {
    ...
    pub fn validate_startup(&self, active_templates: &[crate::workflow::model::WorkflowTemplate]) {
        let mut unregistered = Vec::new();

        for template in active_templates {
            if let Some(graph_value) = &template.graph
                && let Ok(graph) =
                    serde_json::from_value::<crate::workflow::model::WorkflowGraph>(graph_value.clone())
            {
                for node in &graph.nodes {
                    if node.node_type == crate::workflow::model::NodeType::AutoTask
...
impl super::graph_linter::ActionLookup for ActionRegistry {
