    ...
    html! {
        div class="cat-select relative" data-ct=(json) {
            ...
            _="on click if next .cat-dropdown's style's display is 'none' then show next .cat-dropdown then show next .cat-backdrop then call focus(first <input/> in next .cat-dropdown) else hide next .cat-dropdown then hide next .cat-backdrop" {
                ...
                svg class="w-4 h-4 text-muted shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" {
                    path d="M19 9l-7 7-7-7" {}
                }
            }

            // Backdrop
            div class="cat-backdrop fixed inset-0 z-[999]" style="display:none"
            _="on click hide next .cat-dropdown then hide me" {}

            // Dropdown panel
 div class="cat-dropdown absolute top-full left-0 mt-1 w-80 bg-white border border-border rounded-sm shadow-[var(--shadow-card)] z-[1000]" style="display:none" {
