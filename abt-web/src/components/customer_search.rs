//! 客户搜索控件（纯 HTMX、自给自足、只依赖自身 URL）
//!
//! - **一个 URL = 一个组件**：`/api/customer-search` 根据参数渲染**整个组件**
//!   （隐藏 input + 只读显示框 + 弹出面板 + 搜索框 + 结果区）。宿主页面 SSR
//!   调用 `customer_search_field()` 直接渲染，客户端 HTMX 加载同一 URL 也能
...
//!   写入隐藏 input 并 `trigger change` → 宿主表单按客户名筛选。
//! - **面板开关 / 清除**：hyperscript（纯前端 UI 状态，项目惯例）。
//! - **事件隔离**：搜索框 keyup/change/input 事件 `halt`，不冒泡到宿主 DOM。
//!
...
#[typed_path("/api/customer-search")]
pub struct CustomerSearchPath;
...
pub fn router() -> Router<crate::state::AppState> {
    Router::new().route(CustomerSearchPath::PATH, get(search_customers))
}

/// HTMX: 渲染**整个**客户搜索组件（field + 结果区）。
///
...
pub async fn search_customers(
    ctx: RequestContext,
    Query(p): Query<CustomerSearchParams>,
) -> Result<Html<String>> {
    ...
    let items: Vec<CounterpartyResult> = state
        ...
        .search_counterparties(&service_ctx, &mut conn, CounterpartyType::Customer, kw, limit)
        .await
        .unwrap_or_default();
...
/// 渲染完整客户搜索组件：外层 div + 隐藏 input + 只读显示框 + 弹出面板（搜索框 + 结果区）。
///
/// - **SSR**：宿主页面通过 `customer_search_field()` 调用（`items=None`，占位"输入关键词搜索…"）。
/// - **端点**：HTMX 请求 `/api/customer-search` 调用（`items=Some(搜索结果)`）。
///
                ...
                _="on click halt"
            {
                div class="flex items-center gap-2 p-2 border-b border-border-soft" {
                    ...
                    input
                        ...
                        autocomplete="off"
                        hx-get=(CustomerSearchPath::PATH)
...
/// 渲染客户搜索字段：隐藏 input + 只读显示框 + 弹出面板（搜索框 + 结果区）。
///
/// 宿主页面 SSR 时调用，内部委托 `render_component`（`items=None`，占位"输入关键词搜索…"）。
/// 渲染结果与端点 `/api/customer-search` 返回的完全一致——同一段代码。
///
...
/// - `value`：当前选中的客户名（用于回显）
/// - `placeholder`：占位符，如 "客户"
pub fn customer_search_field(
    input_id: &str,
    display_id: &str,
    panel_id: &str,
    results_id: &str,
        ...
        div id=(results_id) {
            @match items {
                ...
                Some(list) if list.is_empty() => {
                    div class="px-3 py-4 text-xs text-muted text-center" { "未找到匹配客户" }
                }
                Some(list) => {
                    @for item in list {
