...
pub fn export_dropdown(items: &[ExportItem]) -> Markup {
 ...
 div class="export-menu absolute right-0 top-full mt-1 bg-white border border-border rounded-sm shadow-[var(--shadow-card)] z-50 min-w-[160px] py-1" style="display:none" {
 @for item in items {
...
fn export_menu_item(item: &ExportItem) -> Markup {
 ...
 html! {
 button type="button" class="w-full text-left px-4 py-2 text-sm text-fg-2 hover:bg-accent-bg hover:text-accent transition-colors cursor-pointer border-none bg-transparent"
 hx-post=(path)
 hx-confirm=(confirm_msg)
 hx-swap="none"
