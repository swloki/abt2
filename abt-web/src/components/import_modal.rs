 ...
 html! {
    div class="flex flex-col gap-3" {
        ...
        a   href=(template_path)
            ...
            download
        { (crate::components::icon::download_icon("w-4 h-4")) " 下载模板" }
        form
            hx-post=(upload_path)
    ...
    div class="import-result" {
        div class="flex gap-4" {
            div class="flex flex-col items-center" {
                span class="import-text-2xl font-bold font-mono tabular-nums text-fg success" {
                    (result.success_count)
                }
                span class="import-text-sm text-muted mt-1" { "成功" }
            }
            div class="flex flex-col items-center" {
                span class="import-text-2xl font-bold font-mono tabular-nums text-fg failed" {
                    (result.failed_count)
                }
                ...
                ul {
                    @for err in &result.row_errors {
                        li {
                            ...
                            "\"："
                            (err.reason)
                            @if let Some(ref v) = err.raw_value { " (" (v) ")" }
                        }
                    }
                }
            }
