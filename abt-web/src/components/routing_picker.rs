                ...
                div class="overflow-y-auto flex-1 min-h-0 p-6" {
                    div class="routing-search-bar flex gap-4 mb-4 pb-4 border-b border-border-soft"
                    {
                        ...
                        div class="flex-1 flex flex-col gap-1" {
                            ...
                            input
                                class="routing-search-input w-full px-3 py-2 border border-border rounded-sm text-sm bg-white text-fg outline-none transition-all duration-150 focus:border-accent"
                                type="text"
                                name="keyword"
                                placeholder="输入工艺路径名称…"
                                hx-get=(RoutingSearchPath::PATH)
                                hx-trigger="keyup changed delay:300ms"
                                hx-sync="this:replace"
                                hx-target="#routing-search-results"
                                hx-swap="innerHTML"
                                hx-include=".routing-search-bar" {}
                        }
                    }
                    div id="routing-search-results"
