...
pub fn work_order_picker_modal(modal_id: &str, target_id: &str, display_id: &str) -> Markup {
 ...
 input class="wo-search-input w-full px-3 py-2 border border-border rounded-sm text-sm bg-white text-fg outline-none transition-all duration-150 focus:border-accent"
 type="text" name="keyword" placeholder="工单号…"
 hx-get=(WorkOrderSearchPath::PATH) hx-trigger="keyup changed delay:300ms" hx-sync="this:replace"
 hx-target="#wo-search-results" hx-swap="innerHTML" hx-include=".wo-search-bar" {}
 }
 div class="flex-1 flex flex-col gap-1" {
 label class="text-xs font-medium text-fg-2" { "产品编码" }
 input class="wo-search-input w-full px-3 py-2 border border-border rounded-sm text-sm bg-white text-fg outline-none transition-all duration-150 focus:border-accent"
 type="text" name="product_code" placeholder="产品编码…"
 hx-get=(WorkOrderSearchPath::PATH) hx-trigger="keyup changed delay:300ms" hx-sync="this:replace"
 hx-target="#wo-search-results" hx-swap="innerHTML" hx-include=".wo-search-bar" {}
 }
 ...
 name="status"
 hx-get=(WorkOrderSearchPath::PATH) hx-trigger="change"
 hx-target="#wo-search-results" hx-swap="innerHTML" hx-include=".wo-search-bar" {
 option value="-1" { "全部" }
 option value="2" { "已计划" }
 option value="3" { "已下达" }
 ...
 div id="wo-search-results" class="max-h-[400px] overflow-y-auto"
 hx-get=(WorkOrderSearchPath::PATH) hx-trigger="intersect once" hx-swap="innerHTML"
...
fn wo_picker_results(
 ...
 data-wid=(wo.id) data-wnum=(wo.doc_number.as_str()) _=(click_hs.clone()) {
 div class="min-w-0" {
 div class="text-sm font-medium text-fg truncate" { (wo.doc_number) }
 div class="text-xs text-muted" { "计划 " (wo.planned_qty) " · 完工 " (wo.completed_qty) " · " (sl) }
 }
