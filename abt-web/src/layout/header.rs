...
pub fn header(claims: &Claims, module_name: &str, page_name: Option<&str>) -> Markup {
    ...
    html! {
        header class="h-[var(--header-h)] bg-white/82 backdrop-blur-md border-b border-border-soft flex items-center justify-between px-8 sticky top-0 z-10" {
            ...
            div class="flex items-center gap-4" {
                button class="w-9 h-9 rounded-md border border-border-soft bg-white/60 grid place-items-center relative cursor-pointer transition-all duration-150 hover:bg-bg hover:border-border hover:shadow-sm [&_svg]:w-4.5 [&_svg]:h-4.5 [&_svg]:text-muted"
                        title="通知" {
                    (icon::bell_icon(""))
