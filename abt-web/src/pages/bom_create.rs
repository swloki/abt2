...
fn bom_create_page(categories: &[BomCategory]) -> Markup {
 ...
 p class="text-muted text-sm mt-1" { "第一步：基本信息" }
 }
 ...
 label { "BOM名称 " span class="text-danger" { "*" } }
 input type="text" name="bom_name" required placeholder="请输入BOM名称" {}
 }
 div class="form-field" {
 label { "BOM分类 " span class="text-danger" { "*" } }
 ...
 a class="inline-flex items-center gap-2 py-[9px] px-[18px] rounded-sm bg-white text-fg-2 border border-border hover:bg-surface hover:border-[rgba(37,99,235,0.3)] hover:text-accent text-sm font-medium cursor-pointer transition-all duration-150 shadow-xs" href=(format!("{}?restore=true", BomListPath::PATH)) { "取消" }
