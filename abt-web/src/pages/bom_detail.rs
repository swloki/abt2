    ...
    div {
        ...
        div class="flex justify-between items-start" {
            div class="flex items-center gap-5" {
                div class="inline-grid place-items-center rounded-full text-white font-semibold shrink-0 select-none bg-purple-100"
            ...
            div class="flex gap-2 flex-wrap" {
                a   class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-white text-fg-2 border border-border hover:bg-surface hover:border-[rgba(37,99,235,0.3)] hover:text-accent cursor-pointer transition-all duration-150 no-underline"
                    href=(format!("{list_path}?restore=true"))
                { (icon::arrow_left_icon("w-3.5 h-3.5")) "返回列表" }
                @if can_view_cost {
                    button
                        type="button"
                        class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-white text-fg-2 border border-border hover:bg-surface hover:border-[rgba(37,99,235,0.3)] hover:text-accent cursor-pointer transition-all duration-150"
                ...
                @if can_edit {
                    a   class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-accent text-white border-none hover:bg-accent-hover cursor-pointer transition-all duration-150 no-underline"
                        href=(BomEditPath { id: bom.bom_id })
                    { (icon::edit_icon("w-3.5 h-3.5")) "编辑" }
                }
                @if can_edit && is_draft {
                    button
                        type="button"
                        class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-accent text-white border-none hover:bg-accent-hover cursor-pointer transition-all duration-150"
                        hx-confirm="确定要发布此 BOM 吗？发布后将无法修改。"
                        hx-post=(publish_path.to_string())
                        hx-swap="none"
                    { (icon::check_circle_icon("w-3.5 h-3.5")) "发布" }
                }
                @if can_delete {
                    button
                        type="button"
                        class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-danger text-white border-none hover:opacity-90 cursor-pointer transition-all duration-150"
                        hx-confirm=({
                            format!(
                                "确定要删除 BOM {} 吗？此操作不可撤销。",
                                bom.bom_name,
                ...
                button
                    ...
                    hx-post=({
                        format!(
                            "{}/bom?bom_id={}",
                            crate::routes::excel::EXPORT_START_PATH,
                            bom.bom_id,
                        )
                    })
        ...
        div class="mt-6 bg-white border border-border-soft rounded-md overflow-hidden" {
            div class="flex items-center justify-between px-5 pt-5 pb-3" {
                ...
                span class="text-muted font-normal text-xs" { "（共 " (node_count) " 个节点）" }
            }
            @if bom.bom_detail.nodes.is_empty() {
                ...
                div class="overflow-x-auto" {
                    table class="w-full border-collapse" {
                        thead {
                            tr {
                                th  class="w-[40px] px-3 py-3 text-center text-xs font-semibold text-white bg-accent whitespace-nowrap"
                                { "编号" }
                                th  class="w-[40px] px-3 py-3 text-center text-xs font-semibold text-white bg-accent whitespace-nowrap"
                                { "层级" }
                                th  class="w-[120px] px-3 py-3 text-left text-xs font-semibold text-white bg-accent whitespace-nowrap"
                                { "产品编码" }
                                th  class="w-[200px] px-3 py-3 text-left text-xs font-semibold text-white bg-accent whitespace-nowrap"
                                { "产品" }
                                th  class="w-[80px] px-3 py-3 text-left text-xs font-semibold text-white bg-accent whitespace-nowrap"
                                { "工作中心" }
                                th  class="w-[80px] px-3 py-3 text-right text-xs font-semibold text-white bg-accent whitespace-nowrap"
                                { "数量" }
                                th  class="w-[60px] px-3 py-3 text-center text-xs font-semibold text-white bg-accent whitespace-nowrap"
                                { "单位" }
                                th  class="w-[50px] px-3 py-3 text-right text-xs font-semibold text-white bg-accent whitespace-nowrap"
                                { "损耗率" }
                                th  class="w-[150px] px-3 py-3 text-left text-xs font-semibold text-white bg-accent whitespace-nowrap"
                                { "备注" }
                            }
                        }
                        tbody {
        ...
        @if can_view_cost {
            ...
            @let cost_overlay_cls = if auto_open == Some("cost") {
                "drawer-overlay open fixed inset-0 z-[1000] flex justify-end bg-[rgba(0,0,0,0.35)]"
            } else {
                "drawer-overlay fixed inset-0 z-[1000] flex justify-end bg-[rgba(0,0,0,0.35)]"
            };
            div id="cost-drawer"
                class=(cost_overlay_cls)
                _="on click[me is event.target] remove .open from me"
            {
                div id="costpanel"
                    class="drawer-panel bg-white h-full w-[1000px] max-w-[100vw] flex flex-col shadow-[-8px_0_30px_rgba(0,0,0,0.1)]"
                    _="on click halt the event on htmx:afterSettle add .open to #cost-drawer"
                {
                    div class="flex items-center justify-between px-6 py-4 border-b border-border-soft sticky top-0 bg-white z-10"
                    {
                        ...
                        button
                            type="button"
                            class="w-8 h-8 border-none bg-transparent cursor-pointer text-muted rounded-md grid place-items-center hover:bg-surface hover:text-fg transition-colors"
        ...
        @if can_view_labor_cost {
            @let labor_overlay_cls = if auto_open == Some("labor") {
                "drawer-overlay open fixed inset-0 z-[1000] flex justify-end bg-[rgba(0,0,0,0.35)]"
            } else {
                "drawer-overlay fixed inset-0 z-[1000] flex justify-end bg-[rgba(0,0,0,0.35)]"
            };
            div id="labor-drawer"
                class=(labor_overlay_cls)
                _="on click[me is event.target] remove .open from me"
            {
...
fn bom_node_row(
 ...
 html! {
    tr class={ (row_class) } {
        td  class="px-3 py-2.5 text-center text-xs font-mono opacity-60 border-b border-border-soft"
        { (index + 1) }
        td class="px-3 py-2.5 text-center border-b border-border-soft" { (level) }
        td  class="px-3 py-2.5 font-mono tabular-nums text-sm whitespace-nowrap border-b border-border-soft"
        { (code) }
        td  class="px-3 py-2.5 text-sm border-b border-border-soft"
            style=(format!("padding-left:{}px", (level - 1) * 24 + 12))
        {
        ...
        td  class="px-3 py-2.5 font-mono tabular-nums text-sm text-right border-b border-border-soft"
        { (node.quantity) }
 ...
 html! {
    div class="space-y-4" {
        ...
        @if !report.warnings.is_empty() {
            div class="border border-warn-400 bg-warn-50 rounded-md" {
                button
                    type="button"
                    class="flex items-center justify-between w-full py-3 px-4 border-none bg-transparent cursor-pointer text-left hover:bg-warn-100 rounded-md"
                    _="on click if next <div/>'s style's display is 'none' then show next <div/> else hide next <div/>"
                {
                    div class="flex items-center gap-2 text-[13px] font-medium text-warn-800" {
                        (icon::circle_alert_icon("w-4 h-4 text-warn"))
                        span { "部分材料缺失单价（共 " (report.warnings.len()) " 项）" }
                    }
                    (icon::chevron_down_icon("w-4 h-4"))
                }
                div class="overflow-hidden border-t border-warn-400" style="display:none" {
                    ul class="list-none m-0 p-0 py-3 px-4" {
                        @for w in &report.warnings {
                            li class="text-[13px] text-warn-800 py-0.5" { "- " (w) }
                        }
                    }
                }
            }
        ...
        // Product code
        div class="bg-slate-50 rounded-md px-4 py-2.5" {
            p class="text-[13px] text-muted m-0" {
                "产品编码："
                span class="font-mono font-semibold text-fg" { (report.product_code) }
            }
        }
        // Summary cards
        div class="grid grid-cols-3 gap-3" {
            // Material cost card
            div class="border border-slate-200 bg-white rounded-md p-4" {
                div class="text-[11px] font-medium text-slate-500 mb-1" { "材料成本" }
                div class="text-lg font-bold tabular-nums text-accent" {
                    (format_currency(material_total))
                }
                div class="text-[11px] text-slate-400 mt-1" { (report.material_costs.len()) " 项材料" }
            }
            // Labor cost card
            div class={
                    ({
                ...
                }
            {
                div class={
                        ({
                            format!(
                                "text-[11px] font-medium mb-1 {}",
                                if all_resolved { "text-accent-500" } else { "text-warn" },
                            )
                        })
                    }
                { "总成本" }
                @if all_resolved {
                    div class={
                            ({
                                format!(
                                    "text-lg font-bold tabular-nums {}",
                                    if all_resolved { "text-accent" } else { "text-warn" },
                                )
                            })
                        }
                    { (format_currency(material_total + labor_total)) }
                } @else {
                    div class="text-lg font-bold text-warn" { "-" }
                }
                div class={
                        ({
                            format!(
        ...
        @if !temp_prices.is_empty() {
            div class="flex items-center gap-2 bg-accent-50 text-xs text-accent-500 px-3 py-2 rounded-sm"
            {
                (icon::circle_alert_icon("w-4 h-4"))
                span {
                    "已使用 "
                    strong { (temp_prices.len()) }
                    " 个临时价格"
                }
                button
                    type="button"
                    class="border-none text-accent-500 text-xs cursor-pointer font-medium bg-transparent"
        ...
        // Material cost table
        div class="mb-4" {
            div class="text-[13px] font-semibold text-slate-700 mb-2" { "【材料成本】" }
            div class="overflow-hidden border border-slate-200 rounded-md" {
                table class="w-full border-collapse" {
                    thead {
                        tr {
                            th  class="text-left text-xs font-medium text-slate-500 bg-slate-50 px-3 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb]"
                            { "产品名称" }
                            th  class="text-left text-xs font-medium text-slate-500 bg-slate-50 px-3 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb]"
                            { "产品编码" }
                            th  class="text-right text-xs font-medium text-slate-500 bg-slate-50 px-3 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb]"
                            { "数量" }
                            th  class="text-right text-xs font-medium text-slate-500 bg-slate-50 px-3 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb]"
                            { "单价" }
                            th  class="text-right text-xs font-medium text-slate-500 bg-slate-50 px-3 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb]"
                    ...
                    tbody {
                        @for item in &report.material_costs {
                            ...
                            @let has_temp = is_missing
                                && temp_prices.contains_key(&item.product_id);
                            @let tr_class = if is_missing && !has_temp {
                                "bg-danger-100 text-danger-800"
                            } else {
                                ""
                            };
                            tr class=(tr_class) {
                                td  class="px-3 py-2 text-sm font-medium truncate max-w-[160px]"
                                    title=(item.product_name)
                                { (item.product_name) }
                                td  class="px-3 py-2 text-sm font-mono tabular-nums text-slate-500 whitespace-nowrap"
                                { (item.product_code) }
                                td  class="px-3 py-2 text-sm text-right font-mono tabular-nums whitespace-nowrap"
                                { (item.quantity) }
                                td class="px-3 py-2 text-sm text-right whitespace-nowrap" {
                                    @if let Some(price) = item.unit_price {
                                        ...
                                        span class="inline-flex items-center gap-1.5" {
                                            span class="font-mono tabular-nums" {
                                                (format_currency(effective_price.unwrap_or(Decimal::ZERO)))
                                            }
                                            span
                                                class="text-[10px] bg-warn-100 text-warn-800 px-1.5 py-0.5 rounded"
                                            { "临时" }
                                        }
                                        form
                                            class="inline-flex"
                                            ...
                                            hx-swap="innerHTML"
                                        {
                                            ...
                                            button
                                                type="submit"
                                                class="border-none bg-danger-50 text-danger-500 w-[22px] h-[22px] text-sm cursor-pointer grid place-items-center rounded"
                                                title="回退"
                                            ...
                                            hx-swap="innerHTML"
                                        {
                                            ...
                                            input
                                                type="text"
                                                class="w-[100px] text-xs bg-white outline-none text-right border border-slate-200 rounded px-2 py-1"
                                                name="temp_price"
                                                placeholder="输入单价"
                                                _="on click halt the event on focus halt the event" {}
                                            button
                                                type="submit"
                                                class="border-none bg-accent text-white w-6 h-6 text-xs cursor-pointer grid place-items-center rounded"
                                                title="确认"
                                                _="on click halt the event then trigger submit on closest <form/>"
                                            { "✓" }
                                        }
                                ...
                                td class="px-3 py-2 text-sm text-right whitespace-nowrap" {
                                    @if let Some(price) = effective_price {
                                        ...
                                        @if has_temp {
                                            span
                                                class="font-mono tabular-nums text-warn font-medium"
                                            { (format_currency(amt)) }
                                        } @else {
                                            span
                                                class="font-mono tabular-nums text-accent font-medium"
                                            { (format_currency(amt)) }
                                        }
                                    } @else {
            ...
            div class="flex items-center justify-end gap-2 bg-accent-50 px-4 py-2.5 rounded-md mt-2"
            {
                span class="text-[13px] font-medium text-slate-700" { "材料成本合计:" }
                span class="text-base font-bold tabular-nums text-accent" id="cost-material-total" {
                    (format_currency(material_total))
                }
            }
        }
        ...
        div class="mb-4" {
            ...
            div class="overflow-x-auto border border-slate-200 rounded-md" {
                table class="w-full border-collapse min-w-[800px]" {
                    thead {
                        tr {
                            th  class="text-left text-xs font-medium text-slate-500 bg-slate-50 px-2 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb]"
                            { "工序名称" }
                            th  class="text-left text-xs font-medium text-slate-500 bg-slate-50 px-2 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb] w-[80px]"
                            { "工作中心" }
                            th  class="text-right text-xs font-medium text-slate-500 bg-slate-50 px-2 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb] w-[70px]"
                            { "标准工时" }
                            th  class="text-center text-xs font-medium text-slate-500 bg-slate-50 px-2 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb] w-[48px]"
                            { "委外" }
                            th  class="text-right text-xs font-medium text-slate-500 bg-slate-50 px-2 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb] w-[80px]"
                            { "单价" }
                            th  class="text-right text-xs font-medium text-slate-500 bg-slate-50 px-2 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb] w-[60px]"
                            { "数量" }
                            th  class="text-right text-xs font-medium text-slate-500 bg-slate-50 px-2 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb] w-[80px]"
                            { "小计" }
                            th  class="text-left text-xs font-medium text-slate-500 bg-slate-50 px-2 py-2 whitespace-nowrap [border-bottom:1px_solid_#e5e7eb]"
                    ...
                    tbody {
                        @if report.labor_costs.is_empty() {
                            tr {
                                td colspan="8" class="text-center text-muted text-sm py-6" {
                                    "暂无人工成本数据"
                                }
                            }
                        } @else {
                            @for item in &report.labor_costs {
                                ...
                                {
                                    td  class="px-2 py-2 text-sm font-medium truncate max-w-[120px]"
                                        title=(item.name)
                                    { (item.name) }
                                    ...
                                    td class="px-2 py-2 text-sm text-center whitespace-nowrap" {
                                        @if item.is_outsourced {
                                            span
                                                class="inline-flex items-center px-1.5 py-0.5 rounded text-[11px] font-medium bg-purple-100 text-purple-700"
                                            { "委外" }
                                        } @else {
                                            span class="text-muted" { "—" }
                                        }
            ...
            div class={
                    ({
                        format!(
                            "flex items-center justify-end gap-2 px-4 py-2.5 rounded-md mt-2 {}",
                            if has_labor_cost_issue {
            ...
            {
                span class="text-[13px] font-medium text-slate-700" { "人工成本合计:" }
                span
                    class={
                        ({
                            format!(
        ...
        // Total footer
        div class="flex items-center justify-end gap-2 bg-slate-100 px-4 py-3 rounded-md" {
            @if !all_resolved {
                ...
                span class="text-[13px] font-medium text-warn" { (total_hint) }
            } @else {
                span class="text-[13px] font-medium text-slate-700" { "总成本:" }
 ...
 html! {
    div class="mb-2" {
        p class="text-sm text-fg-2" {
            "BOM："
            span class="font-medium" { (bom_name) }
        }
    }
    ...
    div class="mb-6" {
        ...
        div class="overflow-x-auto" {
            table class="w-full border-collapse min-w-[800px]" {
                thead {
                    tr {
                        th  class="text-left text-xs font-semibold text-muted py-2 px-2 border-b border-border-soft"
                        { "工序名称" }
                        th  class="text-left text-xs font-semibold text-muted py-2 px-2 border-b border-border-soft w-[80px]"
                        { "工作中心" }
                        th  class="text-right text-xs font-semibold text-muted py-2 px-2 border-b border-border-soft w-[70px]"
                        { "标准工时" }
                        th  class="text-center text-xs font-semibold text-muted py-2 px-2 border-b border-border-soft w-[48px]"
                        { "委外" }
                        th  class="text-right text-xs font-semibold text-muted py-2 px-2 border-b border-border-soft w-[80px]"
                        { "单价" }
                        th  class="text-right text-xs font-semibold text-muted py-2 px-2 border-b border-border-soft w-[60px]"
                        { "数量" }
                        th  class="text-right text-xs font-semibold text-muted py-2 px-2 border-b border-border-soft w-[80px]"
                        { "小计" }
                        th  class="text-left text-xs font-semibold text-muted py-2 px-2 border-b border-border-soft"
                    ...
                    @if report.items.is_empty() {
                        ...
                        @for item in &report.items {
                            ...
                            tr class=(if is_zero { "bg-danger-bg" } else { "" }) {
                                td  class="py-2 px-2 font-medium text-fg border-b border-border-soft truncate max-w-[120px]"
                                    title=(item.name)
                                { (item.name) }
                                ...
                                td  class="py-2 px-2 text-right font-mono tabular-nums text-fg-2 border-b border-border-soft"
                                { (item.quantity) }
        ...
        div class={
                ({
                    format!(
                        ...
                        if has_issue { "bg-danger-bg" } else { "bg-accent-bg" },
                    )
                })
            }
        {
            ...
            span
                class={
                    ({
                        format!(
                            "text-base font-bold {}",
                            if has_issue { "text-danger-500" } else { "text-accent" },
                        )
                    })
                }
