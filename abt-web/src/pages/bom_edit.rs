                ...
                div class="grid grid-cols-2 gap-4 gap-x-6 mb-6" {
                    ...
                    div class="form-field" {
                        ...
                        input
                            type="text"
                            name="work_center"
                            value=(node.work_center.as_deref().unwrap_or("")) {}
                    }
                    div class="form-field" {
                        label { "位置" }
                        input
                            type="text"
                            name="position"
                            value=(node.position.as_deref().unwrap_or("")) {}
                    }
...
fn bom_edit_page(
 ...
 html! {
    div id="bom-edit-app"
        hx-get=(BomEditPath { id: bom.bom_id }.to_string())
        hx-trigger="nodeUpdated from:body"
        hx-select="#bom-edit-app"
        hx-swap="outerHTML"
        hx-disinherit="hx-select"
    {
        ...
        div class="flex items-center justify-between gap-3 mb-4 flex-wrap" {
            div class="flex items-center gap-2 flex-wrap" {
                a   class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-white text-fg-2 border border-border hover:bg-surface hover:border-[rgba(37,99,235,0.3)] hover:text-accent cursor-pointer transition-all duration-150 no-underline icon:w-3.5 icon:h-3.5"
                    href=(format!("{list_path}?restore=true"))
                { (icon::arrow_left_icon("w-3.5 h-3.5")) "返回列表" }
                @if !categories.is_empty() {
                    select
                        name="bom_category_id"
                        class="h-[29px] px-2 text-xs font-medium bg-white border border-border text-fg-2 rounded-sm cursor-pointer outline-none"
                        hx-post=({
                            BomUpdateCategoryPath {
                                id: bom.bom_id,
                            }
                                .to_string()
                        })
            ...
            div class="flex items-center gap-2 flex-wrap" {
                @if !is_draft && is_owner {
                    button
                        class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-white text-fg-2 border border-border hover:bg-surface hover:border-[rgba(37,99,235,0.3)] hover:text-accent cursor-pointer transition-all duration-150 icon:w-3.5 icon:h-3.5"
                        id="bom-publish-btn"
                        _="on click show #bom-publish-dialog"
                    { (icon::return_arrow_icon("w-3.5 h-3.5")) "取消发布" }
                } @else if is_draft {
                    button
                        class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-success text-white border border-transparent hover:opacity-90 cursor-pointer transition-all duration-150 icon:w-3.5 icon:h-3.5"
                        id="bom-publish-btn"
                        _="on click show #bom-publish-dialog"
                        disabled[node_count == 0]
                        title="请先添加物料"
                    { (icon::rocket_icon("w-3.5 h-3.5")) "发布" }
                }
                @if node_count == 0 {
                    button
                        type="button"
                        class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-accent text-accent-on border-none hover:bg-accent-hover cursor-pointer transition-all duration-150 icon:w-3.5 icon:h-3.5"
                        id="bom-add-root-btn"
                        _="on click put '0' into <input[name='parent_id']/>'s value then add .is-open to #bom-add-modal then call bomLoadProducts()"
                    { (icon::plus_icon("w-3.5 h-3.5")) "添加根节点" }
                } @else {
                    button
                        type="button"
                        class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-success text-white border border-transparent hover:opacity-90 cursor-pointer transition-all duration-150 icon:w-3.5 icon:h-3.5"
                    ...
                    { (icon::copy_icon("w-3.5 h-3.5")) "另存为" }
                }
                a   class="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium rounded-sm bg-warn-500 text-white border border-transparent hover:opacity-90 cursor-pointer transition-all duration-150 no-underline icon:w-3.5 icon:h-3.5"
                    href=(format!("/admin/labor/bom-cost/{}", bom.bom_id))
                { (icon::currency_icon("w-3.5 h-3.5")) "人工成本" }
            }
        }
        // ── Title ──
            ...
            div class="flex-1 min-w-0" {
                h1  class="text-2xl font-bold text-fg tracking-tight flex items-center flex-wrap gap-x-3 gap-y-1"
                {
                    span class="font-mono text-sm font-normal text-muted" {
                        (bom.product_code.as_deref().unwrap_or("—"))
                    }
                    span { (bom.bom_name) }
                    span
                        class="text-xs font-normal bg-slate-50 text-fg-2 rounded-sm px-2 py-[2px] whitespace-nowrap"
                    { "v" (bom.version) }
                    span class=({
                        format!(
                            "status-pill {}",
                    ...
                    table class="w-full text-[13px] min-w-[900px]" {
                        thead {
                            tr {
                                ...
                                th  class="w-[40px] px-2 py-3 text-center text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "编号" }
                                th  class="w-[40px] px-2 py-3 text-center text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "层级" }
                                th  class="w-[120px] px-3 py-3 text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "产品编码" }
                                th  class="px-3 py-3 text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "产品" }
                                th  class="w-[100px] px-3 py-3 text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "工作中心" }
                                th  class="w-[80px] px-3 py-3 text-right text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "数量" }
                                th  class="w-[60px] px-3 py-3 text-center text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "单位" }
                                th  class="w-[50px] px-3 py-3 text-right text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "损耗率" }
                                th  class="w-[100px] px-3 py-3 text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "位置" }
                                th  class="w-[90px] px-3 py-3 text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "备注" }
                                th  class="w-[120px] px-3 py-3 text-center text-xs font-semibold text-white whitespace-nowrap bg-accent"
                                { "操作" }
                            }
                        }
                ...
                div class="overflow-y-auto flex-1 min-h-0" {
                    ...
                    div class="flex gap-4 border-b border-border-soft product-search-bar" {
                        ...
                        div class="flex-1 flex flex-col gap-[4px]" {
                            ...
                            input
                                class="w-full pl-9 pr-3 py-2 border border-border rounded-sm text-sm bg-white text-fg outline-none transition-all duration-150 focus:border-accent product-search-input"
                                type="text"
                                name="name"
                                placeholder="输入产品名称…"
                                autocomplete="off"
                        ...
                        button
                            type="button"
                            class="self-end px-4 py-2 border border-border rounded-sm bg-bg text-fg-2 text-sm cursor-pointer whitespace-nowrap hover:bg-surface hover:border-accent transition-all duration-150"
                            hx-get=(BomProductsPath::PATH)
                            hx-target="#bom-edit-product-results"
                            hx-swap="innerHTML"
                            hx-include=".product-search-bar"
                    ...
                    div id="bom-edit-product-results"
                        class="overflow-y-auto min-h-[200px] max-h-[360px]"
                    {
        ...
        ({
            crate::components::confirm_dialog::confirm_dialog(
                ...
                html! {
                    form id = "bom-node-delete-form" class = "hidden" hx - swap = "none"
                    {}
                },
            )
        })
        ...
        @if !is_draft && is_owner {
            ({
                crate::components::confirm_dialog::confirm_dialog(
                    ...
                    "publish-bom-form",
                    html! {
                        form id = "publish-bom-form" class = "hidden" hx - post =
                        (publish_path.to_string()) hx - swap = "none" {}
                    },
                )
            })
        } @else if is_draft {
        ...
        (modal_shell("bom-save-as-modal", "z-[1000]", html! {
            div class="bg-bg rounded-xl w-[680px] max-h-[85vh] flex flex-col overflow-hidden shadow-xl"
            {
                ...
                div class="overflow-y-auto flex-1 min-h-0 p-6" {
                    form hx-post=(BomSaveAsPath { id: bom.bom_id }.to_string()) hx-swap="none" {
                        div class="form-field" {
                            label {
                                "新 BOM 名称 "
                                span class="text-danger" { "*" }
...
fn bom_node_row(
 ...
 html! {
    tr  class=(row_class)
        ...
        data-level=(level)
        data-ancestors=(ancestors_str)
    {
        td class="text-center px-2 py-2.5 border-b border-border-soft" {
            @if has_children {
                button
                    type="button"
                    class="inline-flex items-center justify-center w-[20px] h-[20px] border-none rounded-sm cursor-pointer shrink-0"
        ...
        td  class="px-3 py-2.5 text-xs border-b border-border-soft"
            style=(format!("padding-left:{}px", indent_px))
        { (name) }
        td class="px-3 py-2.5 text-xs border-b border-border-soft" { (work_center) }
        td  class="font-mono tabular-nums text-right px-3 py-2.5 text-xs border-b border-border-soft"
        { (node.quantity) }
        ...
        td class="px-3 py-2.5 text-center border-b border-border-soft" {
            div class="flex gap-1 justify-center" {
                button
                    ...
                    _=({
                        format!(
                            "on click put '{}' into <input[name='parent_id']/>'s value then add .is-open to #bom-add-modal then call bomLoadProducts()",
                            node.id,
                        )
                    })
                ...
                button
                    ...
                    _=({
                        format!(
                            "on click set #bom-node-delete-form's @hx-delete to '/admin/md/boms/{}/nodes/{}' then call htmx.process(document.querySelector('#bom-node-delete-form')) then show #bom-delete-dialog",
                            bom_id,
                            node.id,
                        )
                    })
        ...
        div class="py-2" {
            @for p in products {
                div class="flex items-center justify-between p-3 border-b border-border-soft" {
                    ...
                    form
                        hx-post=(format!("/admin/md/boms/{}/nodes", bid))
                        hx-swap="none"
