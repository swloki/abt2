...
use crate::routes::fms::ArAgingPath;
use crate::utils::RequestContext;
use abt_macros::require_permission;

#[derive(Deserialize, Debug, Default)]
pub struct AgingQuery {
    pub buckets: Option<String>, // comma-separated: "30,60,90,120"
}
        ...
        div id="data-card" class="data-card" {
            div class="overflow-x-auto" {
                table class="data-table" {
                    thead {
                        tr {
                            th class="px-4 py-3 text-left text-xs font-medium text-fg-2 uppercase" { "客户" }
...
pub async fn get_page(
    ...
    let rows = svc.ar_aging(&service_ctx, &mut conn, req).await.unwrap_or_default();

    let content = html! {
        div {
            h1 class="text-xl font-bold text-fg tracking-tight mb-5" { "应收账龄分析" }
            (aging_table(&rows, &bucket_labels))
        }
    };

    let page_html = admin_page(is_htmx, "应收账龄分析", &claims, "finance", ArAgingPath::PATH, "财务管理", None, content, &nav_filter);
