...
fn login_page(error: Option<&str>, username: &str) -> Markup {
 ...
 p class="text-[15px] text-white/55 leading-relaxed mb-10" { "统一管理销售、采购、库存全流程，实时掌控业务数据，让决策更高效。" }

 div class="flex flex-col gap-5" {
 (brand_feature(trending_up_icon("w-[18px] h-[18px] text-white"), "全链路销售管理", "报价 → 订单 → 发货 → 对账，一站式闭环"))
 (brand_feature(clipboard_list_icon("w-[18px] h-[18px] text-white"), "采购协同", "供应商管理、采购订单、付款全流程数字化"))
 (brand_feature(package_icon("w-[18px] h-[18px] text-white"), "实时库存", "多仓库、多品类库存实时可视，自动预警"))
 }
 }
 }
 ...
 button class="w-full py-[11px] bg-bg text-fg-2 border border-border rounded-md text-sm font-medium cursor-pointer flex items-center justify-center gap-[10px] hover:bg-surface transition-colors duration-150" {
...
fn login_form_area(error: Option<&str>, username: &str) -> Markup {
 ...
 button type="submit" class="w-full inline-flex items-center justify-center gap-2 py-[11px] rounded-md bg-accent text-accent-on hover:bg-accent-hover text-sm font-semibold cursor-pointer transition-all duration-150 shadow-[0_4px_14px_rgba(22,119,255,0.3)]" {
 span { "登 录" }
...
fn brand_feature(icon_markup: Markup, title: &str, desc: &str) -> Markup {
 ...
 div class="w-9 h-9 rounded-md bg-white/10 grid place-items-center shrink-0" {
 (icon_markup)
 }
 div {
