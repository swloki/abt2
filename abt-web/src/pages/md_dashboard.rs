...
fn quick_entry_card(href: &str, icon_svg: Markup, title: &str, count: &str, icon_cls: &str) -> Markup {
    html! {
        a href=(href) class="data-card flex flex-col items-center gap-3 p-6 text-center no-underline hover:shadow-[var(--shadow-card-hover)] transition-shadow duration-200" {
...
fn flow_arrow() -> Markup {
    html! {
        svg class="shrink-0 mx-3 text-border" width="48" height="20" viewBox="0 0 48 20" fill="none" stroke="currentColor" stroke-width="1.5" {
