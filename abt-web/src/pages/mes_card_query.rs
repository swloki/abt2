...
pub async fn search_card(
 ...
 _ => {
 return Ok(Html(html! {
    div class="text-center text-muted p-6" { "请输入流转卡序列号进行查询" }
}.into_string()));
 }
 };
 ...
 None => {
 return Ok(Html(html! {
    div class="text-center text-danger p-6" { "未找到流转卡 \"" (query) "\" 对应的批次" }
}.into_string()));
 }
 };
    ...
    div {
        ...
        div class="bg-bg border border-border-soft rounded-lg p-10 text-center relative overflow-hidden"
        {
            ...
            div class="mt-6 flex gap-3 items-center justify-center flex-wrap" {
                form
                    hx-get=(CardQuerySearchPath::PATH)
                ...
                {
                    input
                        class="flex-1 min-w-0 pl-9 pr-3 py-2 border border-border rounded-sm text-sm bg-white text-fg outline-none transition-all duration-150 focus:border-accent"
                        type="text"
                        name="q"
                        placeholder="输入流转卡号 / 批次号，如 FC-SN-060301"
                        autofocus;
                    button
                        class="shrink-0 inline-flex items-center gap-2 py-2 px-4 rounded-sm bg-accent text-accent-on border-none hover:bg-accent-hover text-sm font-medium cursor-pointer transition-all duration-150 shadow-[0_1px_2px_rgba(37,99,235,0.2)]"
                        type="submit"
                    {
                        ({
                            PreEscaped(
                                r#"<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>"#,
                            )
                        })
                        "查询"
                    }
                ...
                button
                    class="shrink-0 inline-flex items-center gap-2 py-2 px-4 rounded-sm bg-white text-fg-2 border border-border hover:bg-surface hover:border-[rgba(37,99,235,0.3)] hover:text-accent text-sm font-medium cursor-pointer transition-all duration-150 shadow-xs"
                {
                    ({
                        PreEscaped(
                            r#"<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 7V5a2 2 0 012-2h2M17 3h2a2 2 0 012 2v2M21 17v2a2 2 0 01-2 2h-2M7 21H5a2 2 0 01-2-2v-2"/><rect x="7" y="7" width="10" height="10" rx="1"/></svg>"#,
                        )
                    })
                    "扫描二维码"
                }
        ...
        @if !recent_batches.is_empty() {
            div class="bg-bg border border-border-soft rounded-lg p-6" {
                div class="text-sm font-semibold text-fg flex items-center gap-2" {
                    ({
                        PreEscaped(
                            r#"<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>"#,
                        )
                    })
                    "最近查询的流转卡"
                }
                div class="grid grid-cols-3 gap-4 mt-4" {
                    @for batch in recent_batches {
                        @let (status_label, status_cls) = batch_status_label(
                            &batch.status,
                        );
                        @let progress_pct = if batch.total_steps.unwrap_or(0) > 0 {
                            ((batch.current_step as f64
                                / batch.total_steps.unwrap_or(1) as f64) * 100.0) as i32
                        } else {
                            0
                        };
                        @let progress_color = match &batch.status {
                            abt_core::mes::enums::BatchStatus::Completed => {
                                "var(--success)"
                            }
                            abt_core::mes::enums::BatchStatus::PendingReceipt => {
                                "var(--warn)"
                            }
                            abt_core::mes::enums::BatchStatus::Suspended => {
                                "var(--danger)"
                            }
                            ...
                            hx-swap="innerHTML"
                        {
                            div class="flex items-center justify-between mb-2" {
                                span class="text-sm font-mono tabular-nums font-semibold text-fg" {
                                    (batch.card_sn)
                                }
                                span
                                    class=({
                                        format!(
                                            "status-pill {}",
                                            crate::utils::status_color(status_cls),
                                        )
                                    })
                                { (status_label) }
                            }
    ...
    div class="bg-bg border border-border-soft rounded-lg p-5 mb-5 shadow-[var(--shadow-card)] overflow-hidden"
    {
        ...
        div class="flex items-center gap-4 text-sm text-muted" {
            span class="inline-flex items-center gap-1" {
                ({
                    PreEscaped(
                        r#"<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="7" width="20" height="14" rx="2"/></svg>"#,
                    )
                })
                "批次 "
                span class="font-mono tabular-nums" { (batch.batch_no) }
            }
            span class="inline-flex items-center gap-1" {
                ({
                    PreEscaped(
                        r#"<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 000 1.4l1.6 1.6a1 1 0 001.4 0l3.77-3.77a6 6 0 01-7.94 7.94l-6.91 6.91a2.12 2.12 0 01-3-3l6.91-6.91a6 6 0 017.94-7.94l-3.76 3.76z"/></svg>"#,
                    )
                })
                "工单 "
            ...
            div class="flex flex-col gap-1" {
                ...
                span class="text-sm text-fg font-medium" {
                    span class="text-success font-mono tabular-nums" {
                        (crate::utils::fmt_qty(batch.completed_qty))
                    }
                    " / "
                    span class="text-danger font-mono tabular-nums" {
                        (crate::utils::fmt_qty(batch.scrap_qty))
                    }
                }
            }
            div class="flex flex-col gap-1" {
        ...
        @if !routings.is_empty() {
            div class="flex items-start relative px-4 py-6 gap-2" {
                @for (i, routing) in routings.iter().enumerate() {
                    ...
                    @let p_completed = progress
                        .map_or(rust_decimal::Decimal::ZERO, |p| p.completed_qty);
                    @let p_defect = progress
                        .map_or(rust_decimal::Decimal::ZERO, |p| p.defect_qty);
                    @let p_work_hours = routing_work_hours
                        .get(&routing.id)
                        .copied()
                    ...
                    div class="flex flex-col items-center text-center shrink-0 min-w-[80px]" {
                        ...
                        div class="text-[11px] text-muted mt-1" {
                            @if is_completed || is_current {
                                "完成 "
                                (crate::utils::fmt_qty(p_completed))
                                @if p_defect > rust_decimal::Decimal::ZERO {
                                    br;
                                    "不良 "
                                    (crate::utils::fmt_qty(p_defect))
                                }
                                br;
                                "工时 "
                                (crate::utils::fmt_qty(p_work_hours))
                                "h"
                            } @else { "待生产" }
                        }
                    }
        ...
        @if !reports.is_empty() {
            div class="bg-bg border border-border-soft rounded-lg mb-0 shadow-[var(--shadow-card)] overflow-hidden"
            {
                div class="p-4 border-b border-border-soft text-sm font-semibold text-fg flex items-center gap-2 bg-surface-raised"
                {
                    ({
                        PreEscaped(
                            r#"<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>"#,
                        )
                    })
                    "报工明细"
                }
                        ...
                        table class="data-table" {
                            ...
                            tbody {
                                @for report in reports {
                                    @let process_name = routing_map
                                        .get(&report.routing_id)
                                        .map(|r| r.process_name.as_str())
                                        .unwrap_or("—");
                                    @let worker_name = user_map
                                        .get(&report.worker_id)
                                        .map(|s| s.as_str())
                                        .unwrap_or("—");
                                    @let unit_price = routing_map
                                        .get(&report.routing_id)
                                    ...
                                    tr {
                                        td {
                                            a   href=(format!("/admin/mes/reports/{}", report.id))
                                                class="text-accent font-mono tabular-nums"
                                            { (report.doc_number) }
                                        }
                                        td { (process_name) }
                                        td {
                                            @if report.shift == abt_core::mes::enums::ShiftType::Day {
                                                "白班"
                                            } @else { "夜班" }
                                        }
                                        td { (worker_name) }
                                        td  class="text-right text-[13px] font-mono tabular-nums text-success"
            ...
            div class="text-center text-muted p-4" { "暂无报工记录" }
        }
    }
}
}
