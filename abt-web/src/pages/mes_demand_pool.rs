 ...
 html! {
    div {
        ...
        div id="demand-pool-data-card" {
            ...
            @match data {
                ViewData::Material { result } => { (material_grid_fragment(result, params)) }
 ...
 html! {
    div class="grid grid-cols-4 gap-4 mb-6" {
        ...
        div class="flex items-center gap-3 bg-bg border border-border-soft rounded-lg px-5 py-4 shadow-[var(--shadow-card)]"
        {
            ...
            div {
                div class="text-xl font-bold font-mono tabular-nums leading-tight text-danger" {
...
fn material_grid_fragment(
 ...
 html! {
    div class="data-card" {
        ...
        @for item in &result.items { (material_row(item)) }
        ...
        div class="p-4" {
            table class="data-table" {
                ...
                tbody id=(format!("expand-tbody-{pid}")) {
                    tr {
                        td colspan="7" class="flex items-center justify-center p-8 text-muted" {
                            "加载中..."
                        }
                    }
                }
        ...
        div class="overflow-x-auto" {
            table class="data-table" {
                    ...
                    tr {
                        th class="w-10" {
                            input
                                type="checkbox"
                                title="全选"
                                onchange="var cb=this;cb.closest('table').querySelectorAll('input.demand-cb:not([disabled])').forEach(function(c){c.checked=cb.checked;c.dispatchEvent(new Event('change',{bubbles:true}))})";
                        }
                        th { "需求ID" }
                        th { "产品编码" }
                        th { "产品名称" }
                ...
                tbody {
                    @if result.items.is_empty() {
                        tr {
                            td colspan="11" class="text-center text-muted" { "暂无需求记录" }
                        }
                    }
                    @for item in &result.items { (detail_row(item)) }
                }
            }
        }
        (detail_batch_bar())
