 ...
 html! {
    div {
            ...
            hx-swap="none"
        {
            ...
            div class="form-section" {
                ...
                div class="grid grid-cols-2 gap-4 gap-x-6 mb-6" {
                    div class="form-field" {
                        ...
                        input
                            class="w-full px-3 py-2 border border-border rounded-sm text-sm bg-surface text-fg transition-all duration-150 outline-none focus:border-accent focus:shadow-[var(--shadow-focus)]"
                            type="text"
                            readonly
                            value=(product_name) {}
                    }
                    div class="form-field" {
                        ...
                        input
                            class="w-full px-3 py-2 border border-border rounded-sm text-sm bg-surface text-fg transition-all duration-150 outline-none focus:border-accent focus:shadow-[var(--shadow-focus)] font-mono tabular-nums"
                            type="text"
                            readonly
                            value=(product_code) {}
                    }
            ...
            div class="form-section" {
                div class="flex items-center gap-2 text-sm font-semibold text-fg mb-4 pb-2 border-b border-border-soft"
                { (icon::clock_icon("w-[18px] h-[18px]")) "默认排程参数" }
                ...
                div class="flex justify-between items-center mb-3" {
                    div class="flex items-center gap-2 text-sm font-semibold text-fg m-0 p-0 border-none"
                    {
                        ...
                        @if let Some(pid) = product_id {
                            span class="font-normal text-muted ml-2" { "(物料 ID: " (pid) ")" }
                        }
                    }
                    button
                        type="button"
                    ...
                    ({
                        PreEscaped(
                            r#"<script>document.getElementById('applyDefaultBtn').addEventListener('click',function(){
 ...
 if(inputs[1]&&end) inputs[1].value=end;
                ...
                div class="overflow-x-auto" {
                    table class="data-table" {
                        ...
                        tbody id="demand-tbody" {
                            @for d in demands { (demand_row(d, preselected_ids)) }
                            @if demands.is_empty() {
                                tr {
                                    td colspan="8" class="text-center text-muted py-8" { "暂无待处理需求" }
                                }
                            }
                        }
                    }
            ...
            div class="sticky bottom-0 flex items-center justify-end gap-3 px-6 py-4 bg-bg border-t border-border-soft"
            {
                ...
                div class="flex gap-3" {
                    ...
                    { (icon::send_icon("w-4 h-4")) "创建草稿" }
                    button
                        type="submit"
                        name="action"
                        value="release"
                        class="inline-flex items-center gap-2 py-[9px] px-[18px] rounded-sm bg-accent text-accent-on border-none hover:bg-accent-hover text-sm font-medium cursor-pointer transition-all duration-150 shadow-[0_1px_2px_rgba(37,99,235,0.2)] bg-[linear-gradient(135deg,var(--accent),#6366f1)]"
                        hx-confirm="创建后将自动确认并下达，生成工单（含工序、批次）。继续？"
                        hx-disabled-elt="this"
                    { (icon::rocket_icon("w-4 h-4")) "创建并下达" }
                }
            }
        }
...
fn demand_row(d: &DemandSummary, preselected_ids: &[i64]) -> Markup {
 ...
 html! {
    tr  class=@if is_checked { "demand-row-selected" }
    {
        td {
            @if is_pending {
                input type="checkbox" value=(d.id) checked[is_checked];
        ...
        td {
            input
                class="w-full px-3 py-2 border border-border rounded-sm text-sm bg-white text-fg transition-all duration-150 outline-none focus:border-accent focus:shadow-[var(--shadow-focus)] text-xs w-[130px] px-1.5 py-1"
                type="date"
                value=(row_start) {}
        }
        td {
        ...
        td {
            button
                ...
                title="移除"
                _="on click remove closest <tr/> then call updateDemandSummary()"
