...
pub async fn load_usage_data(
 ...
 None => {
 return Ok(Html(html! {
    div class="bg-bg border border-border-soft rounded-md p-5 mb-5 shadow-[var(--shadow-sm)] text-center py-8 text-sm text-muted"
    { "请选择工单" }
}.into_string()));
 }
 };
 ...
 html! {
    div {
        ...
        div class="flex items-center gap-3 mb-5 flex-wrap" {
            select
                class="w-60 px-3 py-2 border border-border rounded-sm text-sm bg-white text-fg outline-none transition-all duration-150 focus:border-accent cursor-pointer"
                name="wo_id"
...
fn usage_data_fragment(wo_info: &abt_core::mes::dashboard::model::WoBasicInfo, ctx: &MaterialUsageContext) -> Markup {
    ...
    let status_pill = html! {
        span class=(format!("text-[11px] px-2 py-0.5 rounded-full font-medium {}", pill_cls)) {
            (status_label)
        }
    };
    ...
    html! {
            ...
            div class="flex gap-4 text-sm text-muted" {
                span {
                    ...
                    strong class="font-mono tabular-nums" {
                        (crate::utils::fmt_qty(wo_info.planned_qty))
                    }
                }
                span {
                    "完成: "
                    strong class="text-success font-mono tabular-nums" {
                        (crate::utils::fmt_qty(wo_info.completed_qty))
                    }
                }
        ...
        div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5" {
            ...
            div class="flex items-center gap-4 p-5 bg-bg border border-border-soft rounded-md" {
                ...
                div {
                    div class="text-lg font-bold font-mono tabular-nums text-fg" {
                        (crate::utils::fmt_qty(ctx.standard_qty))
                    }
            ...
            div class="flex items-center gap-4 p-5 bg-bg border border-border-soft rounded-md" {
                ...
                div {
                    div class="text-lg font-bold font-mono tabular-nums text-fg" {
                        (crate::utils::fmt_qty(ctx.picked_total))
                    }
            ...
            div class="flex items-center gap-4 p-5 bg-bg border border-border-soft rounded-md" {
                ...
                div {
                    div class="text-lg font-bold font-mono tabular-nums text-fg" {
                        (crate::utils::fmt_qty(ctx.backflush_qty))
                    }
            ...
            div class="flex items-center gap-4 p-5 bg-bg border border-border-soft rounded-md" {
                ...
                div {
                    @let variance_cls = if ctx.variance > Decimal::ZERO {
                        "text-danger"
                    } else if ctx.variance < Decimal::ZERO {
                        "text-success"
                    } else {
                        ""
                    };
                    ...
                    @if ctx.standard_qty > Decimal::ZERO {
                        @let rate = ((ctx.variance / ctx.standard_qty) * Decimal::ONE_HUNDRED)
                            .abs();
                        div class="text-xs text-muted mt-1" {
                            "超出标准 "
        ...
        @if !ctx.bom_items.is_empty() {
            div class="data-card mb-5" {
                ...
                { (icon::box_icon("w-5 h-5")) "BOM 标准用量 vs 实际消耗" }
                    ...
                    table class="data-table" {
                        ...
                        tbody {
                            @for item in ctx.bom_items {
                                ...
                                @let diff_cls = if diff > Decimal::ZERO {
                                    "text-danger"
                                ...
                                @let loss_rate = if item.standard_total > Decimal::ZERO {
                                    let r = ((item.picked_qty - item.standard_total)
                                        / item.standard_total) * Decimal::ONE_HUNDRED;
                                ...
                                tr {
                                    td class="font-mono tabular-nums" {
                                        (item.component_code.as_deref().unwrap_or("—"))
                                    }
                                    ...
                                    td class="text-right text-[13px] font-mono tabular-nums" {
                                        (crate::utils::fmt_qty(item.per_unit_qty))
                                    }
                                    td class="text-right text-[13px] font-mono tabular-nums" {
                                        (crate::utils::fmt_qty(item.standard_total))
                                    }
                                    td class="text-right text-[13px] font-mono tabular-nums" {
                                        (crate::utils::fmt_qty(item.picked_qty))
                                    }
                                    td class="text-right text-[13px] font-mono tabular-nums" {
                                        (crate::utils::fmt_qty(item.backflush_total))
                                    }
                                    td class="text-right text-[13px] font-mono tabular-nums" {
                                        (loss_rate)
                                    }
        ...
        @if !ctx.bf_records.is_empty() {
            div class="data-card mb-5" {
                div class="flex items-center gap-2 px-5 py-4 text-sm font-semibold text-fg border-b border-border-soft"
                { (icon::refresh_icon("w-5 h-5")) "倒冲明细记录" }
                div class="overflow-x-auto" {
                    table class="data-table" {
                        thead {
                            tr {
                        ...
                        tbody {
                            @for rec in ctx.bf_records {
                                tr {
                                    td class="font-mono tabular-nums" {
                                        a   href=(format!("/admin/wms/backflush/{}", rec.id))
                                            class="text-accent font-medium cursor-pointer"
                                        { (rec.doc_number) }
                                    }
                                    td class="text-right text-[13px] font-mono tabular-nums" {
                                        (crate::utils::fmt_qty(rec.completed_qty))
                                    }
        ...
        @if !ctx.requisitions.is_empty() {
            div class="data-card mb-5" {
                div class="flex items-center gap-2 px-5 py-4 text-sm font-semibold text-fg border-b border-border-soft"
                { (icon::clipboard_list_icon("w-5 h-5")) "领料记录" }
                div class="overflow-x-auto" {
                    table class="data-table" {
                        thead {
                            tr {
                        ...
                        tbody {
                            @for req in ctx.requisitions {
                                tr {
                                    td class="font-mono tabular-nums" {
                                        a   href=(format!("/admin/wms/requisition/{}", req.id))
                                            class="text-accent font-medium cursor-pointer"
                                        { (req.doc_number) }
                                    }
...
fn backflush_status_label(s: &abt_core::wms::enums::BackflushStatus) -> Markup {
    ...
    html! {
        span class=(format!("text-[11px] px-2 py-0.5 rounded-full font-medium {}", cls)) { (label) }
    }
}
