...
fn receipt_list_page(
 ...
 @if can_create {
 a class="inline-flex items-center gap-2 py-[9px] px-[18px] rounded-sm bg-accent text-accent-on border-none hover:bg-accent-hover text-sm font-medium cursor-pointer transition-all duration-150 shadow-[0_1px_2px_rgba(37,99,235,0.2)]" href=(ReceiptCreatePath::PATH) { (icon::plus_icon("w-4 h-4")) "新建入库" }
 }
 }}
...
fn receipt_table_fragment(
 ...
 hx-push-url="true" {
 div class="relative flex-1 max-w-xs [&_[class*=i-lucide]]:absolute [&_[class*=i-lucide]]:left-3 [&_[class*=i-lucide]]:top-1/2 [&_[class*=i-lucide]]:-translate-y-1/2 [&_[class*=i-lucide]]:w-4 [&_[class*=i-lucide]]:h-4 [&_[class*=i-lucide]]:text-muted" {(icon::search_icon(""))
 input class="pl-9 pr-3 py-2 border border-border rounded-sm text-sm bg-white text-fg outline-none transition-all duration-150 focus:border-accent w-[180px] search-input" type="text" name="keyword" placeholder="搜索单号…" value=(params.keyword.as_deref().unwrap_or(""));
 }
 }
...
fn receipt_data_card(
 ...
 tr { td colspan="8" class="text-center text-muted py-8" { "暂无入库记录" } }
 }
 }}
 }
