    ...
    html! {
        div class="min-h-screen flex flex-col bg-surface" {
            ...
            div class="flex items-center gap-2 px-8 py-5" {
                span
                    class="w-8 h-8 rounded-md bg-accent text-white grid place-items-center font-bold text-sm"
                { "A" }
            ...
            main class="flex-1 grid place-items-center px-6 py-10" {
                div class="text-center max-w-md" {
                    ...
                    div class="flex items-center justify-center gap-3 mt-8" {
                        a   href="/"
                            class="inline-flex items-center gap-2 px-5 py-2.5 rounded-sm bg-accent text-white text-sm font-medium no-underline hover:bg-accent-hover transition-colors"
                        { (PreEscaped(home_icon)) "返回首页" }
                        button
                            type="button"
