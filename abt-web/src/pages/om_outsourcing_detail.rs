...
fn status_pill(label: &str, class: &str) -> Markup {
 html! {
    span class=(format!("status-pill {}", crate::utils::status_color(class))) { (label) }
}
}
    ...
    div class="bg-bg border border-border-soft rounded-xl relative overflow-hidden mb-7 shadow-[var(--shadow-card)]"
    {
        ...
        div class="flex items-center justify-between px-8 py-5 border-b border-border-soft" {
            div class="flex items-center gap-3" {
                div class="w-10 h-10 rounded-xl grid place-items-center bg-[linear-gradient(135deg,rgba(217,119,6,0.08),rgba(37,99,235,0.08))]"
                {
                    ({
                        maud::PreEscaped(
                            r#"<svg class="w-5 h-5 text-warn" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>"#,
                        )
                    })
                }
                span class="text-[18px] font-bold text-fg" { "发料明细" }
                span class="text-xs text-muted bg-surface px-2 py-0.5 rounded-full" {
                    (format!("{} 项物料", materials.len()))
                }
            }
        }
            ...
            table class="data-table" {
                ...
                tbody {
                    @if materials.is_empty() {
                        tr {
                            td colspan="7" class="text-center text-muted py-8" { "暂无发料明细" }
                        }
                    } @else {
                        @for m in materials {
                            tr {
                                td {
                                    span class="font-semibold text-fg" { "产品 #" (m.product_id) }
                                }
                                td class="text-right font-mono tabular-nums" {
                                    (crate::utils::fmt_qty(m.planned_qty))
                                }
                                td class="text-right font-mono tabular-nums text-success" {
                                    (crate::utils::fmt_qty(m.sent_qty))
                                }
                                td class="text-right font-mono tabular-nums" {
                                    (crate::utils::fmt_qty(m.returned_qty))
                                }
                                td  class="text-right font-mono tabular-nums text-warn font-semibold"
                                { (crate::utils::fmt_qty(m.sent_qty - m.returned_qty)) }
                                td class="text-right font-mono tabular-nums" {
                                    (crate::utils::fmt_qty(m.unit_cost))
                                }
        ...
        div class="flex items-center justify-end gap-8 px-8 py-4 border-t border-border-soft bg-surface"
        {
            div class="flex flex-col items-end gap-0.5" {
                ...
                span class="text-lg font-bold font-mono tabular-nums text-warn" {
                    (crate::utils::fmt_qty(in_transit_amount))
                }
            }
            div class="flex flex-col items-end gap-0.5" {
                span class="text-xs text-muted font-semibold" { "加工费" }
                span class="text-lg font-bold font-mono tabular-nums text-accent" {
                    (crate::utils::fmt_qty(processing_fee))
                }
            }
        }
    }
        ...
        div class="flex items-center justify-between px-8 py-5 border-b border-border-soft" {
            div class="flex items-center gap-3" {
                div class="w-10 h-10 rounded-xl grid place-items-center bg-[linear-gradient(135deg,rgba(22,163,74,0.08),rgba(37,99,235,0.08))]"
                {
                    ({
                        maud::PreEscaped(
                            r#"<svg class="w-5 h-5 text-success" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 7h12M8 12h12M8 17h12M4 7h.01M4 12h.01M4 17h.01"/></svg>"#,
                        )
                    })
                }
                span class="text-[18px] font-bold text-fg" { "收发记录" }
                span class="text-xs text-muted bg-surface px-2 py-0.5 rounded-full" {
                    (format!("{} 条记录", records.len()))
                }
            }
        }
        div class="overflow-x-auto px-8 pb-5" {
            table class="data-table" {
                ...
                tbody {
                    @if records.is_empty() {
                        tr {
                            td colspan="5" class="text-center text-muted py-8" { "暂无收发记录" }
                        }
                    } @else {
                        @for r in records {
                            ...
                            tr {
                                ...
                                td  class={
                                        ...
                                        ({
                                            if r.quantity.is_zero() {
                                                ""
                                            } else if r.quantity > rust_decimal::Decimal::ZERO {
                                                "text-success"
                                            } else {
                                                "text-danger"
                                            }
            ...
            div class="px-10 py-8" {
                    ...
                    div {
                        div class="text-[24px] font-bold text-fg flex items-center gap-[14px]" {
                            div class="doc-icon [&_svg]:w-[22px] [&_svg]:h-[22px] [&_svg]:text-accent"
                            {
                                ({
                                    maud::PreEscaped(
                                        r#"<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 16V4m0 0L3 8m4-4l4 4M17 8v12m0 0l4-4m-4 4l-4-4"/></svg>"#,
                                    )
                                })
                            }
                            (order.doc_number)
                    ...
                    div class="flex gap-[8px] shrink-0" {
                        ...
                        @if {
                            ...
                            button
                                ...
                                _="on click add .is-open to #record-node-modal"
                            {
                                ({
                                    maud::PreEscaped(
                                        r#"<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-[15px] h-[15px]"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg>"#,
                                    )
                                })
                                "记录节点"
                            }
                        ...
                        @if order.status == OutsourcingStatus::Sent {
                            button
                                class="inline-flex items-center gap-2 py-[9px] px-[18px] rounded-sm bg-white text-fg-2 border border-border hover:bg-surface hover:border-[rgba(37,99,235,0.3)] hover:text-accent text-sm font-medium cursor-pointer transition-all duration-150 shadow-xs"
                                _="on click add .is-open to #receive-modal"
                            {
                                ({
                                    maud::PreEscaped(
                                        r#"<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-[15px] h-[15px]"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>"#,
                                    )
                                })
                                "收货登记"
                            }
                        ...
                        @if {
                            ...
                            button
                                class="inline-flex items-center gap-2 py-[9px] px-[18px] rounded-sm bg-white text-fg-2 border border-border hover:bg-surface hover:border-[rgba(37,99,235,0.3)] hover:text-accent text-sm font-medium cursor-pointer transition-all duration-150 shadow-xs"
                                _="on click add .is-open to #convert-modal"
                            {
                                ({
                                    maud::PreEscaped(
                                        r#"<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-[15px] h-[15px]"><path d="M7 16V4m0 0L3 8m4-4l4 4M17 8v12m0 0l4-4m-4 4l-4-4"/></svg>"#,
                                    )
                                })
                                "转自制"
                            }
                        ...
                        @if order.status == OutsourcingStatus::Draft {
                            button
                                class="inline-flex items-center gap-2 py-[9px] px-[18px] rounded-sm bg-white text-fg-2 border border-border hover:bg-surface hover:border-[rgba(37,99,235,0.3)] hover:text-accent text-sm font-medium cursor-pointer transition-all duration-150 shadow-xs text-danger border-[rgba(220,38,38,0.3)]"
                                _="on click add .is-open to #cancel-modal"
                            {
                                ({
                                    maud::PreEscaped(
                                        r#"<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-[15px] h-[15px]"><circle cx="12" cy="12" r="10"/><path d="M15 9l-6 6M9 9l6 6"/></svg>"#,
                                    )
                                })
                                "取消"
                            }
                    ...
                    div {
                        div class="grid grid-cols-3 gap-[20px 48px]" {
                            ...
                            div class="flex flex-col gap-[6px]" {
                                ...
                                span class="text-[15px] text-fg font-semibold" { (work_order_name) }
                            }
                            div class="flex flex-col gap-[6px]" {
                            ...
                            div class="flex flex-col gap-[6px]" {
                                ...
                                span
                                    class="text-[15px] text-fg font-semibold font-mono tabular-nums"
                                {
                                    ({
                                        order
                                            .scheduled_date
                        ...
                        div class="flex flex-wrap gap-x-6 gap-y-1 py-2 text-sm" {
                            ...
                            span class="flex gap-[6px] text-xs text-muted" {
                                "单价 "
                                strong class="font-mono tabular-nums" {
                                    (crate::utils::fmt_qty(order.unit_price))
                                }
                            }
                            span class="flex gap-[6px] text-xs text-muted" {
                                "总金额 "
                    ...
                    div class="flex flex-col items-center justify-center border-l" {
                        div class="flex flex-col items-center gap-[8px]" {
                            div class="w-[56px] h-[56px] relative" {
                                ...
                                span
                                    class="absolute inset-0 grid place-items-center text-[14px] font-bold text-accent font-mono"
                                { (format!("{:.0}%", pct)) }
                            }
                ...
                @if !order.remark.is_empty() {
                    div style="margin-top:20px;padding-top:16px;border-top:1px dashed var(--border-soft)"
                    {
                        ...
                        p class="text-fg-2 text-[13px]" style="margin-top:6px;line-height:1.6" {
                            (&order.remark)
                        }
                    }
                }
        ...
        div class="bg-bg border border-border-soft rounded-xl relative overflow-hidden mb-7 shadow-[var(--shadow-card)]"
        {
            ...
            div class="flex items-center justify-between px-10 pt-8 pb-4" {
                div class="text-[18px] font-bold text-fg flex items-center gap-[14px]" {
                    div class="tracking-icon-wrap [&_svg]:w-5 [&_svg]:h-5 [&_svg]:text-success" {
                        ({
                            maud::PreEscaped(
                                r#"<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>"#,
                            )
                        })
                    }
                    "追踪节点"
            ...
            div class="relative pl-11 before:content-[''] before:absolute before:left-[17px] before:top-[18px] before:bottom-[18px] before:w-0.5 before:rounded-sm before:bg-[linear-gradient(180deg,var(--success)_0%,var(--success)_38%,var(--accent)_50%,var(--border)_55%,var(--border-soft)_100%)] pb-8"
            {
                @for (i, nt) in all_node_types.iter().enumerate() {
                    ...
                    @let is_active = !is_completed && i > 0
                        && tracked_nodes.contains_key(&all_node_types[i - 1]);
                    @let dot_cls = if is_completed {
                        "track-dot completed"
                    } else if is_active {
                        "track-dot active"
                    } else {
                            ...
                            })
                        {
                            div class="flex-1" {
                                    ...
                                    })
                                {
                                    ...
                                    @if is_active {
                                        span
                                            class="ml-2 text-[11px] font-medium text-accent px-2.5 py-0.5 rounded-full bg-[rgba(37,99,235,0.1)]"
                                        { "当前" }
                                    }
                                }
                                @if let Some(t) = tracked {
                                    ...
                                    @if let Some(remark) = &t.remark {
                                        div class="text-xs text-fg-2 bg-bg rounded-sm inline-flex items-start gap-[8px]"
                                        { (remark) }
                                    }
                                } @else {
                                    @if let Some(t) = {
                                        tracked_nodes
                                            .get(&all_node_types[if i > 0 { i - 1 } else { 0 }])
                                    } {
                            ...
                            div class="track-status" {
                                @if is_completed { (status_pill("已完成", "status-completed")) } @else if is_active {
            ...
            div class="bg-bg border border-border-soft rounded-xl overflow-hidden mb-7 shadow-[var(--shadow-card)]"
            {
                div class="text-base font-semibold text-fg px-8 py-5 border-b border-border-soft" {
                    div class="section-icon-wrap [&_svg]:w-4.5 [&_svg]:h-4.5 [&_svg]:text-accent" {
                        ({
                            maud::PreEscaped(
                                r#"<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 7h12M8 12h12M8 17h12M4 7h.01M4 12h.01M4 17h.01"/></svg>"#,
                            )
                        })
                    }
                    "收发记录"
                                ...
                                @for t in tracking {
                                    tr {
                                        ...
                                        td {
                                            ...
                                            } @else { (status_pill("计划中", "status-draft")) }
                                        }
                                    }
                                }
                            }
                ...
                style="width:520px"
            {
                div class="px-6 py-5 border-b border-border-soft flex justify-between items-center shrink-0"
                {
                    ...
                    button
                        class="btn btn-text inline-flex items-center gap-2 rounded-sm text-sm font-medium cursor-pointer whitespace-nowrap relative [&_svg]:w-4 [&_svg]:h-4"
                        type="button"
                        _="on click remove .is-open from #record-node-modal"
                    { "✕" }
                }
                form
                    hx-post=(OmRecordNodePath { id: order.id }.to_string())
                    hx-swap="none"
                            ...
                            strong class="text-accent" {
                                @if let Some(last) = tracking.last() {
                                    @if let Some(next) = {
                                        all_node_types
                                            .iter()
                                            .find(|nt| nt.as_i16() > last.node_type.as_i16())
                                    } { (node_type_label(next)) } @else { "已全部完成" }
                                } @else { "发料" }
                            }
                        }
            ...
            div class="bg-bg rounded-xl w-[680px] max-h-[85vh] flex flex-col overflow-hidden shadow-xl"
            {
                div class="px-6 py-5 border-b border-border-soft flex justify-between items-center shrink-0"
                {
                    ...
                    button
                        class="btn btn-text inline-flex items-center gap-2 rounded-sm text-sm font-medium cursor-pointer whitespace-nowrap relative [&_svg]:w-4 [&_svg]:h-4"
                        type="button"
                        _="on click remove .is-open from #receive-modal"
                    { "✕" }
                }
                form
                    hx-post=({
                        OmOutsourcingReceivePath {
                            id: order.id,
                        }
                            .to_string()
                    })
                ...
                style="width:520px"
            {
                div class="px-6 py-5 border-b border-border-soft flex justify-between items-center shrink-0"
                {
                    ...
                    button
                        class="btn btn-text inline-flex items-center gap-2 rounded-sm text-sm font-medium cursor-pointer whitespace-nowrap relative [&_svg]:w-4 [&_svg]:h-4"
                        type="button"
                        _="on click remove .is-open from #convert-modal"
                    { "✕" }
                }
                form
                    hx-post=({
                        OmOutsourcingConvertPath {
                            id: order.id,
                        }
                            .to_string()
                    })
                    ...
                    hx-on::after-request="if(event.detail.xhr.status<400){document.querySelector('#convert-modal').classList.remove('is-open');this.reset()}"
                {
                    div class="overflow-y-auto flex-1 min-h-0 p-6" class="text-center" class="p-8" {
                            ...
                            style="height:64px;border-radius:50%;background:linear-gradient(135deg,rgba(217,119,6,0.08),rgba(217,119,6,0.15));place-items:center;margin:0 auto var(--space-5)"
                        {
                            ({
                                maud::PreEscaped(
                                    r#"<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="var(--warn)" stroke-width="2"><path d="M7 16V4m0 0L3 8m4-4l4 4M17 8v12m0 0l4-4m-4 4l-4-4"/></svg>"#,
                                )
                            })
                        }
                    ...
                    div class="px-6 py-4 border-t border-border-soft flex justify-end gap-3 shrink-0"
                    {
                        ...
                        button
                            type="submit"
                            class="inline-flex items-center gap-2 py-[9px] px-[18px] rounded-sm bg-accent text-accent-on border-none hover:bg-accent-hover text-sm font-medium cursor-pointer transition-all duration-150 shadow-[0_1px_2px_rgba(37,99,235,0.2)]"
                            style="background:linear-gradient(135deg,var(--warn),#f59e0b)"
                        { "确认转自制" }
                    }
                }
            }
                ...
                style="width:480px"
            {
                div class="px-6 py-5 border-b border-border-soft flex justify-between items-center shrink-0"
                {
                    ...
                    button
                        class="btn btn-text inline-flex items-center gap-2 rounded-sm text-sm font-medium cursor-pointer whitespace-nowrap relative [&_svg]:w-4 [&_svg]:h-4"
                        type="button"
                        _="on click remove .is-open from #cancel-modal"
                    { "✕" }
                }
                form
                    hx-post=({
                        OmOutsourcingCancelPath {
                            id: order.id,
                        }
                            .to_string()
                    })
                    ...
                    hx-on::after-request="if(event.detail.xhr.status<400){document.querySelector('#cancel-modal').classList.remove('is-open');this.reset()}"
                {
                    div class="overflow-y-auto flex-1 min-h-0 p-6" class="text-center" class="p-8" {
                            ...
                            style="height:64px;border-radius:50%;background:linear-gradient(135deg,rgba(220,38,38,0.08),rgba(220,38,38,0.15));place-items:center;margin:0 auto var(--space-5)"
                        {
                            ({
                                maud::PreEscaped(
                                    r#"<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="var(--danger)" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M15 9l-6 6M9 9l6 6"/></svg>"#,
                                )
                            })
                        }
                        p   class="font-bold text-fg"
