        ...
        div class="customer-list-panel" {
               ...
               
            {
                ...
                input
                    ...
                    title="结束日期";
                a   href=(PriceHistoryListPath::PATH)
            ...
            _="on keydown[event.key is 'Escape'] from body remove .open"
        {
                ...
                _="on click halt the event"
            {
                div class="flex items-center justify-between border-b border-border-soft shrink-0" {
                    h2 { "变更详情" }
                    button
                        class="w-[32px] h-[32px] border-none cursor-pointer flex items-center justify-center text-muted"
                        _="on click remove .open from #detail-drawer"
    ...
    div class="data-card" {
        div class="overflow-x-auto" {
            table class="data-table w-full" style="table-layout:fixed" {
                ...
                tbody {
                    @for (i, row) in rows.iter().enumerate() { (price_history_row(i, row)) }
                    @if rows.is_empty() {
                        tr {
                            td colspan="10" class="text-center text-muted py-8" { "暂无价格变更记录" }
                        }
                    }
                }
            }
 ...
 html! {
        ...
        _="on 'htmx:afterRequest' add .open to #detail-drawer"
    {
        ...
        td class="overflow-hidden whitespace-nowrap text-ellipsis" title=(row.product_name) {
            a   href="#"
                class="text-accent cursor-pointer font-medium no-underline"
        ...
        td {
            button
                class="inline-flex items-center gap-2 py-1 px-2.5 rounded-sm bg-white text-fg-2 border border-border hover:bg-surface hover:border-[rgba(37,99,235,0.3)] hover:text-accent text-xs font-medium cursor-pointer transition-all duration-150 shadow-xs"
 ...
 html! {
    ...
    div class="mb-5" {
        ...
        div class="bg-accent-50 flex items-center gap-4 rounded-md" {
            ...
            div class="text-accent text-[20px]" { (icon::arrow_right_icon("w-6 h-6")) }
            div class="text-center" {
    ...
    div class="mb-5" {
        ...
        div class="text-fg font-medium text-[15px]" { (row.created_at.format("%Y-%m-%d %H:%M")) }
    }
}
}
