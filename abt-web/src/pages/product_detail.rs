        ...
        div class="bg-white border border-border-soft rounded p-5" {
            ...
            ({
                detail_row(
                    "产品编码",
                    html! {
                        span class = "font-mono tabular-nums" { (product.product_code) }
                    },
                )
            })
            ({
            ...
            ({
                detail_row(
                    "规格型号",
                    html! {
                        @ if product.meta.specification.is_empty() { "—" } @ else { (&
            ...
            ({
                detail_row(
                    "获取途径",
                    html! {
                        (acquire_channel_label(product.acquire_channel))
                    },
                )
            })
            ({
                detail_row(
                    "产品状态",
                    html! {
                        span class = (format!("status-pill {}", crate
                        ::utils::status_color(status_class))) { (status_label) }
                    },
                )
            })
            ({
                detail_row(
                    "创建时间",
                    html! {
                        @ if let Some(dt) = product.created_at { (dt
                        .format("%Y-%m-%d %H:%M")) } @ else { "—" }
                    },
                )
            })
            ({
                detail_row(
                    "更新时间",
                    html! {
                        @ if let Some(dt) = product.updated_at { (dt
                        .format("%Y-%m-%d %H:%M")) } @ else { "—" }
                    },
                )
            })
        ...
        div class="bg-white border border-border-soft rounded p-5" {
            ...
            ({
                detail_row(
                    "备注",
                    html! {
                        @ if let Some(r) = & product.meta.remark { (r) } @ else { "—" }
                    },
                )
            })
        }
            ...
            div class="flex flex-col gap-[3px]" {
                ...
                div class="text-sm text-fg font-medium flex items-center gap-2" {
                    div class="inline-flex bg-surface border border-border rounded-full gap-[2px]" {
                        span
                            class=({
                                if mode == MaterialConsumptionMode::Backflush {
                                    "px-3 py-1 text-xs rounded-full bg-accent text-accent-on"
                                } else {
                                    "px-3 py-1 text-xs rounded-full bg-surface text-muted"
                                }
                            })
                        { "倒冲" }
                        span
                            class=({
                                if mode == MaterialConsumptionMode::Picking {
                                    "px-3 py-1 text-xs rounded-full bg-accent text-accent-on"
                                } else {
                                    "px-3 py-1 text-xs rounded-full bg-surface text-muted"
                                }
    ...
    div class="bg-bg border border-border-soft rounded-lg p-6" {
        ...
        div class="grid gap-5" {
            ...
            div class="flex flex-col gap-[3px]" {
                ...
                span
                    class="text-sm text-fg font-medium flex items-center gap-2 font-mono tabular-nums"
                { (bom_node_count) }
            }
            div class="flex flex-col gap-[3px]" {
                ...
                span
                    class="text-sm text-fg font-medium flex items-center gap-2 font-mono tabular-nums"
                { (routing.map_or(0, |rd| rd.steps.len())) }
            }
        }
    }
    // ── 使用情况（BOM 引用）──
    ...
    div {
        ...
        div class="flex items-center justify-between mb-6" {
            a   class="inline-flex items-center gap-2 text-sm text-muted hover:text-accent transition-colors duration-150"
                href=(detail_path)
            { (icon::arrow_left_icon("w-4 h-4")) "返回产品详情" }
            h1 class="text-xl font-bold text-fg tracking-tight" { "编辑产品" }
        }

        form id="product-edit-form" hx-post=(update_path) hx-swap="none" {
            // ── Section: 基本信息 ──
                ...
                div class="grid grid-cols-2 gap-4 gap-x-6 mb-6" {
                    ...
                    div class="form-field" {
                        ...
                        input
                            ...
                            name="specification"
                            placeholder="请输入规格型号"
                            value=(product.meta.specification) {}
                    }
                    div class="form-field" {
                        label {
                            "计量单位 "
                        ...
                        select name="unit" required {
                            ...
                            option
                                value="kg"
                                selected[product.unit == "kg" || product.unit == "千克"]
                            { "千克 (kg)" }
                            option value="g" selected[product.unit == "g" || product.unit == "克"] {
                                "克 (g)"
                            }
                            option value="m" selected[product.unit == "m" || product.unit == "米"] {
                                "米 (m)"
                            }
                            option
                                value="cm"
                                selected[product.unit == "cm" || product.unit == "厘米"]
                            { "厘米 (cm)" }
                            option value="L" selected[product.unit == "L" || product.unit == "升"] {
                                "升 (L)"
                            }
                    ...
                    div class="form-field" {
                        ...
                        select name="material_consumption_mode" {
                            option value="backflush" selected[mcm_val == "backflush"] {
                                "倒冲 (backflush)"
                            }
