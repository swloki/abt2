 ...
 html! {
    ...
    div class="grid grid-cols-5 gap-4 mb-8" {
        ({
            stat_card(
                ...
                &stats.active_suppliers.to_string(),
                "活跃供应商",
            )
        })
        ({
            stat_card(
                ...
                &stats.pending_quotations.to_string(),
                "待比价报价",
            )
        })
        ({
            stat_card(
                ...
                "bg-accent-bg text-accent",
                &stats.in_progress_orders.to_string(),
                "进行中订单",
            )
        })
        ({
        ...
        ({
            stat_card(
                ...
                &stats.returns_in_progress.to_string(),
                "退货处理中",
            )
        })
    }
    ...
    div class="grid grid-cols-2 gap-6 mb-8" {
        ...
        div {
            ...
            div class="data-card" {
                ...
                @if stats.returns_in_progress > 0 {
                    ({
                        todo_item(
                            "status-info",
                            "退货中",
                            &format!("{} 笔退货处理中", stats.returns_in_progress),
                            "需处理",
                            false,
                        )
                    })
                ...
                @if stats.draft_recons > 0 {
                    ({
                        todo_item(
                            "status-progress",
                            "对账中",
                            &format!("{} 份对账单待确认", stats.draft_recons),
                            "需处理",
                            false,
                        )
                    })
                ...
                @if {
                    stats.pending_quotations == 0 && stats.in_progress_orders == 0
                        && stats.returns_in_progress == 0 && stats.draft_recons == 0
                        && stats.draft_payments == 0 && stats.pending_misc == 0
                } { ({
                    todo_item(
                        "status-completed",
                        "无待办",
        ...
        div {
            ...
            div class="grid grid-cols-2 gap-3" {
                ...
                ({
                    quick_link_card(
                        "/admin/purchase/reconciliations",
                        "采购对账",
                        "对账管理",
                    )
                })
    ...
    div class="bg-bg border border-border-soft rounded-md p-5 mb-8 shadow-sm" {
        ...
        div class="flex items-center justify-center flex-wrap gap-3 py-6" {
            ...
            ({
                flow_circle(
                    &icon::clipboard_document_icon("w-5 h-5"),
                    "采购订单",
                    "下达采购",
                    "bg-success-bg",
                    "text-success",
        ...
        div class="flex flex-wrap mt-4 justify-center gap-6" {
            ...
            a   href="/admin/purchase/returns"
                class="flex items-center gap-2 text-xs font-medium text-danger no-underline hover:underline"
            {
                svg width="14"
                    height="14"
                    viewBox="0 0 24 24"
            ...
            a   href="/admin/purchase/misc-requests"
                class="flex items-center gap-2 text-xs font-medium text-success no-underline hover:underline"
            {
                svg width="14"
                    height="14"
                    viewBox="0 0 24 24"
        ...
        div class="data-card" {
            ...
            ({
                activity_row(
                    "status-progress",
                    "报价",
                    "供应商「昆山宏达钢材」提交了新的采购报价 PQ-2026-05-00089",
                    "",
                    "2 小时前",
                    false,
                )
            ...
            ({
                activity_row(
                    "status-progress",
                    "退货",
                    "采购退货 PRT-2026-05-00012 已发出，等待供应商确认",
                    "",
                    "昨天",
                    false,
                )
...
fn todo_item(status_key: &str, status_text: &str, desc: &str, time: &str, last: bool) -> Markup {
 ...
 html! {
    div class=({
            format!(
                "flex items-center gap-3 px-5 py-4 cursor-pointer hover:bg-accent-bg transition-colors{border}",
            )
        })
    {
        span class=(format!("status-pill {} text-[11px]", crate::utils::status_color(status_key))) {
            (status_text)
        }
 ...
 html! {
    div class=(format!("flex items-center gap-4 px-5 py-4{border}")) {
        span
            class=({
                format!(
                    "status-pill {} text-[11px] min-w-[56px] justify-center",
                    crate::utils::status_color(status_key),
                )
            })
        { (pill_text) }
...
fn quick_link_card(href: &str, title: &str, count: &str) -> Markup {
 html! {
    a   href=(href)
        class="flex flex-col gap-1 p-4 bg-bg border border-border-soft rounded-md no-underline cursor-pointer hover:border-accent hover:bg-accent-bg transition-colors"
    {
