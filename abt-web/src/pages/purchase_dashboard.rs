...
fn purchase_dashboard_content(stats: &DashboardStats) -> Markup {
 ...
 div class="grid" class="gap-4 mb-8" class="grid-cols-5" {
 // 活跃供应商
 ...
 div {
 div class="text-2xl font-bold font-mono tabular-nums tabular-nums text-fg" { (stats.active_suppliers.to_string()) }
 ...
 div {
 div class="text-2xl font-bold font-mono tabular-nums tabular-nums text-fg" { (stats.pending_quotations.to_string()) }
 ...
 div {
 div class="text-2xl font-bold font-mono tabular-nums tabular-nums text-fg" { (stats.in_progress_orders.to_string()) }
 ...
 div {
 div class="text-2xl font-bold font-mono tabular-nums tabular-nums text-fg" { (stats.returns_in_progress.to_string()) }
 ...
 // ── 2-column: 待办事项 + 快捷入口 ──
 div class="grid" class="mb-8" style="grid-template-columns:1fr 1fr;gap:var(--space-6)" {
 // 待办事项
 div {
 ...
 div class="grid" class="gap-3" style="grid-template-columns:1fr 1fr" {
 ...
 div class="bg-bg border border-border-soft rounded-md p-5 mb-5 shadow-[var(--shadow-sm)]" class="mb-8" {
 div class="bg-bg border border-border-soft rounded-md p-5 mb-5 shadow-[var(--shadow-sm)]-title" { "采购业务流程" }
 div class="flex items-center justify-center flex-wrap" class="gap-3" style="padding:var(--space-6) 0" {
 // 供应商
 ...
 // Branch links
 div class="flex mt-4 justify-center" style="gap:var(--space-6)" {
 div class="flex items-center text-xs text-muted" class="gap-2" {
 ...
 a href="/admin/purchase/demand-pool" class="font-medium" style="color:#7c3aed" { "采购需求池（销售订单驱动）" }
 }
 div class="flex items-center text-xs text-muted" class="gap-2" {
 ...
 a href="/admin/purchase/returns" class="text-danger font-medium" { "采购退货（逆向）" }
 }
 div class="flex items-center text-xs text-muted" class="gap-2" {
 ...
 a href="/admin/purchase/misc-requests" class="text-success font-medium" { "零星请购（非生产物资）" }
 }
 }
 }
 ...
 // ── 最近活动 ──
 div {
 h2 class="font-semibold mb-4" class="text-lg" { "最近活动" }
 div class="data-card" {
...
fn flow_circle(
 ...
 html! {
 div class="flex items-center" class="flex-col gap-2" style="min-width:100px" {
...
fn activity_row(
 ...
 html! {
 div class="flex items-center" class="gap-4" class="px-5 py-4" style="border-bottom:1px solid var(--border-soft)" {
 span class=(pill_class) class="text-[11px] justify-center" class="min-w-[56px]" { (pill_text) }
 span class="flex-1" {
 (desc)
 ...
 span class="text-muted" class="text-xs" { (time) }
 }
 }
}
...
fn activity_row_last(
 ...
 html! {
 div class="flex items-center" class="gap-4" class="px-5 py-4" {
 span class=(pill_class) class="text-[11px] justify-center" class="min-w-[56px]" { (pill_text) }
 span class="flex-1" {
 (desc)
 @if !highlight.is_empty() {
...
fn todo_item(status_class: &str, status_text: &str, desc: &str, time: &str) -> Markup {
 ...
 span class=(status_class) class="text-[11px]" { (status_text) }
 span class="flex-1" { (desc) }
 span class="text-muted" class="text-xs" { (time) }
 }
 }
}
...
fn todo_item_last(status_class: &str, status_text: &str, desc: &str, time: &str) -> Markup {
 html! {
 div class="flex items-center cursor-pointer" class="gap-3" class="px-5 py-4" {
 span class=(status_class) class="text-[11px]" { (status_text) }
 span class="flex-1" { (desc) }
 span class="text-muted" class="text-xs" { (time) }
 }
