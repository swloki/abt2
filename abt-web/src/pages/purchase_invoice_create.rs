...
use abt_core::gl::purchase_invoice::model::{
    CreatePurchaseInvoiceReq, PurchaseInvoiceItemInput,
};
...
use crate::routes::gl::{PurchaseInvoiceCreatePath, PurchaseInvoiceListPath};
...
pub async fn get_create(
    _path: PurchaseInvoiceCreatePath,
    ctx: RequestContext,
) -> Result<Html<String>> {
    ...
    let content = invoice_create_page(&suppliers.items, &products.items, &tax_rates);
...
pub async fn create(
    ...
    let items: Vec<ItemJson> = match serde_json::from_str(&form.items_json) {
        Ok(v) => v,
        Err(_) => {
            return Ok(axum::response::Redirect::to(PurchaseInvoiceCreatePath::PATH).into_response())
        }
    };

    let parsed_items: Vec<PurchaseInvoiceItemInput> = items
        .into_iter()
    ...
    let supplier_id: i64 = match form.supplier_id.trim().parse() {
        Ok(v) if v > 0 => v,
        _ => return Ok(axum::response::Redirect::to(PurchaseInvoiceCreatePath::PATH).into_response()),
    };
    ...
    let req = CreatePurchaseInvoiceReq {
        supplier_id,
        issue_date,
        items: parsed_items,
    ...
    match svc.create(&service_ctx, &mut conn, req).await {
        Ok(_) => Ok(axum::response::Redirect::to(PurchaseInvoiceListPath::PATH).into_response()),
        Err(_) => Ok(axum::response::Redirect::to(PurchaseInvoiceCreatePath::PATH).into_response()),
    }
}
...
fn invoice_create_page(
    ...
    html! {
        div {
            a href=(format!("{}?restore=true", PurchaseInvoiceListPath::PATH))
            ...
            div class="mb-6" {
                h1 class="text-xl font-bold text-fg tracking-tight" { "新建采购发票" }
            }

            form id="purchase-invoice-form"
                hx-post=(PurchaseInvoiceCreatePath::PATH)
                hx-swap="none"
                onsubmit="lineItemCalc('#purchase-invoice-item-tbody').collectItems()" {
                    ...
                    div class="grid grid-cols-2 gap-4 gap-x-6" {
                        div class="form-field" {
                            label class="block text-xs font-medium text-fg-2 mb-1 whitespace-nowrap" {
                                "供应商 " span class="text-danger" { "*" }
                            }
                            select class="w-full px-3 py-2 border border-border rounded-sm text-sm bg-white text-fg outline-none transition-all duration-150 focus:border-accent cursor-pointer" name="supplier_id" required {
                                option value="" { "请选择供应商" }
                                @for s in suppliers {
                ...
                div class="form-section" {
                    ...
                    div class="overflow-x-auto" {
                        table class="w-full border-separate border-spacing-0 min-w-[870px]" {
                            ...
                            tbody id="purchase-invoice-item-tbody" {}
                        }
                    }
                    div class="py-4" {
                        button type="button"
...
function addInvoiceLine() {{
    var tbody = document.getElementById('purchase-invoice-item-tbody');
    ...
    row.innerHTML =
        ...
        + '<td class="px-3 py-2 border-b border-border-soft"><input class="w-[80px] text-right px-2.5 py-1.5 border border-border rounded-sm text-sm font-mono outline-none focus:border-accent" type="number" name="quantity" placeholder="0" oninput="lineItemCalc(\'#purchase-invoice-item-tbody\').calcRow(this.closest(\'tr\'))"></td>'
        + '<td class="px-3 py-2 border-b border-border-soft"><input class="w-[100px] text-right px-2.5 py-1.5 border border-border rounded-sm text-sm font-mono outline-none focus:border-accent" type="number" name="unit_price" placeholder="0.00" oninput="lineItemCalc(\'#purchase-invoice-item-tbody\').calcRow(this.closest(\'tr\'))"></td>'
