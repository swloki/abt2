...
use abt_core::gl::purchase_invoice::{PurchaseInvoice, PurchaseInvoiceItem, PurchaseInvoiceService};
...
use crate::routes::gl::{
    GlEntryDetailPath, PurchaseInvoiceCancelPath, PurchaseInvoiceDetailPath, PurchaseInvoiceListPath,
...
pub async fn get_detail(path: PurchaseInvoiceDetailPath, ctx: RequestContext) -> Result<Html<String>> {
    ...
    let supplier_name = state
        .supplier_service()
        .get(&service_ctx, &mut conn, invoice.supplier_id)
        .await
        .map(|s| s.name)
        .unwrap_or_else(|_| format!("#{}", invoice.supplier_id));
    ...
    let content = detail_page(&invoice, &items, &supplier_name, &product_map);
    let current_path = PurchaseInvoiceDetailPath { id: path.id }.to_string();
    let page_html = admin_page(
        ...
        &current_path,
        "总账管理",
        Some(PurchaseInvoiceListPath::PATH),
        content,
        &nav_filter,
    );
...
pub async fn post(path: PurchaseInvoicePostPath, ctx: RequestContext) -> Result<impl IntoResponse> {
    let RequestContext {
        mut conn,
        state,
        service_ctx,
    ...
    state
        ...
        .await?;
    let redirect = PurchaseInvoiceDetailPath { id: path.id }.to_string();
    Ok(([("HX-Redirect", redirect)], Html(String::new())))
}

#[require_permission("GL", "update")]
pub async fn cancel(path: PurchaseInvoiceCancelPath, ctx: RequestContext) -> Result<impl IntoResponse> {
    let RequestContext {
        mut conn,
        state,
        service_ctx,
        ...
        div {
            ...
            div class="bg-bg border border-border-soft rounded-md p-5 mb-5 shadow-[var(--shadow-sm)]" {
                ...
                div class="grid gap-4 grid-cols-2 md:grid-cols-3" {
                    ...
                    div class="flex flex-col gap-1" {
                        label class="text-xs text-fg-2" { "供应商" }
                        span { (supplier_name) }
                    }
                    div class="flex flex-col gap-1" {
