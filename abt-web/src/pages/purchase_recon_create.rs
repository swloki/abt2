        ...
        form id="precon-form" hx-post=(PreconCreatePath::PATH) hx-swap="none" {
            ...
            div class="data-card" class="mb-4" {
                ...
                div class="grid grid-cols-2 gap-4 gap-x-6 mb-6" {
                    ...
                    div class="form-field field-full" {
                        ...
                        textarea
                            name="remark"
                            placeholder="输入对账单相关备注信息…"
                            class="w-full resize-y"
                            class="rounded-sm"
                            class="border border-border text-sm"
                            style="min-height:60px;padding:8px 12px;font-family:inherit" {}
                    }
                }
            }
            // ── 对账明细 ──
            div class="data-card" class="p-0 overflow-hidden mb-4" {
                div class="flex justify-between items-center" class="px-5 pt-5 pb-3" {
                    ...
                    { (icon::plus_icon("w-3.5 h-3.5")) "从订单添加" }
                }
                // Empty state
                div id="precon-items-empty"
                    class="text-center"
                    style="padding:var(--space-10) var(--space-5)"
                {
                    div class="text-muted mb-4" { "暂无对账明细" }
                    button
                        type="button"
                        class="inline-flex items-center gap-2 py-[9px] px-[18px] rounded-sm bg-accent text-accent-on border-none hover:bg-accent-hover text-sm font-medium cursor-pointer transition-all duration-150 shadow-[0_1px_2px_rgba(37,99,235,0.2)] icon:w-4 icon:h-4"
                        _="on click add .is-open to #order-modal"
                    { (icon::plus_icon("w-3.5 h-3.5")) "选择订单" }
                }
