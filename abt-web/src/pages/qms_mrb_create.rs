...
fn mrb_create_page(products: &[Product], failed_results: &[InspectionResult]) -> Markup {
 ...
 input class="w-full pl-7 pr-3 py-2 border border-border rounded-sm text-sm bg-white text-fg transition-all duration-150 outline-none focus:border-accent" type="number" step="any" name="cost_impact"
 placeholder="0.00";
 }
 }
 }
