    ...
    div {
        ...
        div class="bg-bg border border-border-soft rounded-md p-5 mb-5 shadow-[var(--shadow-sm)]" {
            ...
            div class="grid gap-4" {
                ...
                div class="flex flex-col gap-1" {
                    ...
                    span
                        class=({
                            format!(
                                "status-pill {}",
                                crate::utils::status_color(severity_class),
                            )
                        })
                    { (severity_text) }
                }
                ...
                div class="flex flex-col gap-1" {
                    ...
                    span {
                        ({
                            rma.linked_inspection_result_id
                                .map(|id| id.to_string())
                                .unwrap_or_else(|| "—".into())
                        })
                    }
        ...
        div class="bg-bg border border-border-soft rounded-md p-5 mb-5 shadow-[var(--shadow-sm)]" {
            ...
            div class="grid gap-4" {
                div class="flex flex-col gap-1" {
                    label { "根本原因" }
                    span { (rma.root_cause.as_deref().unwrap_or("待填写")) }
                }
                div class="flex flex-col gap-1" {
                    label { "纠正措施" }
                    span { (rma.corrective_action.as_deref().unwrap_or("待填写")) }
                }
            }
        }
        // ── 其他信息 ──
        div class="bg-bg border border-border-soft rounded-md p-5 mb-5 shadow-[var(--shadow-sm)]" {
            ...
            div class="grid gap-4" {
                div class="flex flex-col gap-1" {
                    label { "备注" }
                    span { (or_dash(&rma.remark)) }
                }
                div class="flex flex-col gap-1" {
                    label { "创建时间" }
                    span { (rma.created_at.format("%Y-%m-%d %H:%M")) }
                }
            }
        }
    }
