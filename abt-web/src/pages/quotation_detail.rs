...
pub async fn submit_quotation(
 ...
 svc.submit(&service_ctx, &mut tx, path.id).await?;
 tx.commit().await
     .map_err(|e| abt_core::shared::types::error::DomainError::Internal(e.into()))?;
...
pub async fn accept_quotation(
 ...
 let mut tx = state.pool.begin().await
     .map_err(|e| abt_core::shared::types::error::DomainError::Internal(e.into()))?;
 svc.accept(&service_ctx, &mut tx, path.id).await?;
 tx.commit().await
     .map_err(|e| abt_core::shared::types::error::DomainError::Internal(e.into()))?;
...
pub async fn reject_quotation(
 ...
 let mut tx = state.pool.begin().await
     .map_err(|e| abt_core::shared::types::error::DomainError::Internal(e.into()))?;
 svc.reject(&service_ctx, &mut tx, path.id).await?;
 tx.commit().await
     .map_err(|e| abt_core::shared::types::error::DomainError::Internal(e.into()))?;
...
fn quotation_detail_page(
 ...
 html! {
    div {
        ...
        { (icon::arrow_left_icon("w-4 h-4")) "返回报价单列表" }
        // ── Detail Header ──
                ...
                div class="flex gap-3" {
                    ...
                    span class="text-[20px] font-bold text-fg text-success" {
                        ({
                            format!(
                                "{:.1}%",
                                q.estimated_margin * rust_decimal::Decimal::ONE_HUNDRED,
                            )
                        })
                    }
                }
