        ...
        }
    {
        ...
        div class="bg-white border border-border-soft rounded p-5" class="mb-5" {
            ...
            div class="flex py-2 text-sm" {
                ...
                span class="detail-value" {
                    @if supplier.lead_time_days > 0 { (supplier.lead_time_days) " 天" } @else {
                        "—"
                    }
                }
            }
        ...
        ({
            crate::components::modal::modal(
                ...
                html! {
                    div class = "grid grid-cols-2 gap-4 gap-x-6 mb-6" { div class =
                    "form-field" { label { "开户银行 *" } input type = "text" name =
                    "bank_name" required placeholder = "请输入开户银行"; } div
                    class = "form-field" { label { "账户名称 *" } input type = "text"
                    name = "account_name" required placeholder = "请输入账户名称";
                    } div class = "form-field field-full" { label { "银行账号 *" }
                    input type = "text" name = "account_number" required placeholder =
                    "请输入银行账号"; } div class = "form-field" { label class =
                    "checkbox-label" { input type = "checkbox" name = "is_default" value
                    = "true"; "默认账户" } } }
                },
            )
        })
 ...
 html! {
        ...
        hx-trigger="contactChanged from:body"
    {
        div class="flex items-center justify-between text-sm font-semibold mb-4 pb-2 border-b border-border-soft"
        {
            ...
            { (icon::plus_icon("w-3.5 h-3.5")) "添加联系人" }
        }
        @if contacts.is_empty() {
            ...
            table class="data-table compact" {
                ...
                tbody {
                    @for c in contacts { (contact_row(c, detail_path, can_delete)) }
                }
            }
        }
    }
 ...
 html! {
        ...
        hx-trigger="bankAccountChanged from:body"
    {
        div class="flex items-center justify-between text-sm font-semibold mb-4 pb-2 border-b border-border-soft"
        {
            ...
            { (icon::plus_icon("w-3.5 h-3.5")) "添加账户" }
        }
        @if bank_accounts.is_empty() {
            ...
            table class="data-table compact" {
                ...
                tbody {
                    @for ba in bank_accounts { (bank_account_row(ba, detail_path, can_delete)) }
                }
            }
        }
    }
