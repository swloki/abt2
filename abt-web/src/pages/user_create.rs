...
fn user_create_page(roles: &[Role], departments: &[Department]) -> Markup {
 html! {
    div {
        ...
        form id="user-form" hx-post=(UserCreatePath::PATH) hx-swap="none" {
            ...
            div class="sticky bottom-0 flex items-center justify-end gap-3 px-6 py-4 bg-bg border-t border-border-soft"
            {
                ...
                button type="submit" class=(BTN_PRIMARY) { (icon::check_circle_icon("w-4 h-4")) "保存" }
            }
        }
    }
}
        ...
        div class="grid grid-cols-2 gap-4 gap-x-6 mb-2" {
            ...
            div class="flex flex-col" {
                ...
                div class="relative" {
                    ...
                    button
                        class="absolute right-2 top-1/2 -translate-y-1/2 text-muted hover:text-fg cursor-pointer bg-transparent border-none p-1"
                        type="button"
 ...
 html! {
    div class=(SECTION) {
        ...
        p class="text-[13px] text-muted mb-4 leading-relaxed" {
            "数据范围由角色与部门自动派生：超级管理员=All，分配部门=Department，否则=Self。无需手动选择。"
        }
