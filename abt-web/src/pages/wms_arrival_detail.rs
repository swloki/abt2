...
fn workflow_steps(status: ArrivalStatus) -> Markup {
 ...
 html! {
    ...
    div class="flex items-center flex-wrap gap-4 mt-3" {
        ...
        span class="inline-flex items-center gap-1 text-xs text-warn" {
            "● 部分接收 (Partially Accepted)"
        }
...
fn arrival_detail_page(
 ...
 html! {
    div {
            ...
            div class="grid gap-5 [grid-template-columns:repeat(auto-fill,minmax(200px,1fr))]" {
                ...
                div class="flex flex-col gap-1" {
                    ...
                    span class="text-sm text-fg font-mono tabular-nums" {
                        (notice.arrival_date.format("%Y-%m-%d"))
                    }
                }
                div class="flex flex-col gap-1" {
                    span class="text-xs text-muted font-medium" { "送货单号" }
                    span class="text-sm text-fg font-mono tabular-nums" {
                        (notice.delivery_note.as_deref().unwrap_or("—"))
                    }
                }
                div class="flex flex-col gap-1" {
                ...
                table class="data-table" {
                    ...
                    tbody {
                        @for (i, item) in items.iter().enumerate() {
                            @let (code, name, spec, unit) = product_info
                                .get(&item.product_id)
                                .map(|(c, n, s, u)| (
                                    c.as_str(),
                                    n.as_str(),
                            ...
                            tr {
                                ...
                                td class="text-right text-[13px] font-mono tabular-nums" {
                                    (format!("{:.2}", item.declared_qty))
                                }
                                td class="text-right text-[13px] font-mono tabular-nums" {
                                    (format!("{:.2}", item.received_qty))
                                }
                                td class="text-right text-[13px] font-mono tabular-nums" {
                                    (format!("{:.2}", item.accepted_qty))
                                }
                        ...
                        @if items.is_empty() {
                            tr {
                                td colspan="9" class="text-center text-muted py-8" { "暂无物料明细" }
                            }
                        }
                    }
                }
        ...
        // ── IQC 质检结果区 ──
        @if {
            is_inspecting || notice.status == ArrivalStatus::Accepted
                || notice.status == ArrivalStatus::PartiallyAccepted
                || notice.status == ArrivalStatus::Rejected
        } {
            div class="bg-bg border border-border-soft rounded-lg p-6 mb-6 shadow-[var(--shadow-card)] [border-left:3px_solid_var(--warn)]"
            {
                div class="text-base font-semibold text-fg mb-4 pb-3 border-b border-border-soft flex items-center gap-2"
                {
                    ...
                    "IQC质检结果"
                    span
                        class="inline-flex items-center gap-[5px] rounded-full text-xs font-medium whitespace-nowrap bg-warn-bg text-warn ml-2"
                    { "检验中" }
                }
...
fn arrival_action_buttons(status: ArrivalStatus, detail_path: &str) -> Markup {
 ...
 html! {
    ...
    button
        ...
        hx-redirect=(detail_path)
    { (icon::check_circle_icon("w-4 h-4")) "确认收货" }
}
 }
 ...
 html! {
    ...
    button
        ...
        hx-redirect=(detail_path)
    { (icon::clipboard_list_icon("w-4 h-4")) "检验接收" }
}
 }
 _ => html! {},
 }
