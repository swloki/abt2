        ...
        div class="bg-bg border border-border-soft rounded-lg p-6 mb-6 shadow-[var(--shadow-card)]"
        {
            ...
            div class="grid gap-5 [grid-template-columns:repeat(auto-fill,minmax(200px,1fr))]" {
                ...
                div class="flex flex-col gap-1" {
                    ...
                    span class="text-sm text-fg font-mono tabular-nums" {
                        (format!("{:.2}", record.completed_qty))
                    }
                }
                div class="flex flex-col gap-1" {
                    span class="text-xs text-muted font-medium" { "倒冲日期" }
                    span class="text-sm text-fg font-mono tabular-nums" {
                        (record.backflush_date.to_string())
                    }
                }
                div class="flex flex-col gap-1" {
                    span class="text-xs text-muted font-medium" { "差异阈值" }
                    span class="text-sm text-fg font-mono tabular-nums" {
                        (format!("{:.2}%", record.variance_threshold))
                    }
                }
                div class="flex flex-col gap-1" {
            ...
            div class="overflow-x-auto" {
                table class="data-table" {
                    ...
                    tbody {
                        ...
                        @if items.is_empty() {
                            tr {
                                td colspan="9" class="text-center text-muted py-8" { "暂无明细数据" }
                            }
                        }
                    }
                }
        ...
        @if !items.is_empty() {
            div class="grid grid-cols-3 gap-4 mt-5" {
                ...
                div class="text-center p-4 bg-surface border border-border-soft rounded-md" {
                    div class=({
                            format!(
                                ...
                                if over_count > 0 { "text-danger" } else { "text-fg" },
                            )
                        })
                    { (over_count) }
                ...
                div class="text-center p-4 bg-surface border border-border-soft rounded-md" {
                    ...
                    {
                        @if max_rate > Decimal::ZERO { "+" (format!("{:.2}", max_rate)) "%" } @else {
                            "0%"
                        }
                    }
 ...
 html! {
    tr {
        ...
        td class="text-right text-[13px] font-mono tabular-nums" {
            (format!("{:.2}", item.theoretical_qty))
        }
        td class="text-right text-[13px] font-mono tabular-nums" {
            (format!("{:.2}", item.actual_qty))
        }
        ...
        td class=(format!("text-right text-[13px] font-mono tabular-nums{variance_cls}")) {
            (rate_sign)
            (format!("{:.2}", item.variance_rate))
            "%"
        }
        td class="text-right text-[13px]" {
            @if item.is_over_threshold {
                span
                    class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-danger-bg text-danger text-xs font-bold"
                { "✓" }
            } @else {
 ...
 html! {
    div class="flex items-center gap-2 mt-6 mb-6" {
        @for (i, (label, _)) in steps.iter().enumerate() {
            ...
            @let dot_cls = if i < current_idx {
                "bg-success"
            } else if i == current_idx {
                "bg-accent ring-[3px] ring-[rgba(37,99,235,0.1)]"
            } else {
                "bg-slate-300"
            };
