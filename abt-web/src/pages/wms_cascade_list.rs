 ...
 html! {
    div {
        ...
        div class="flex items-center gap-3 bg-bg border border-border-soft rounded-md py-5 px-6 mb-6"
        {
            ...
            { (icon::search_icon("w-4 h-4")) "查询" }
        }
        // ── Results ──
        div class="cascade-results" {
            @if let Some(r) = result { (cascade_results(r)) } @else {
                div class="text-center py-8 text-muted" { "请输入产品编码进行查询" }
            }
        }
    }
}
...
fn cascade_results(result: &CascadeInventoryResult) -> Markup {
 html! {
    div {
        ...
        @for group in &result.bom_groups { (bom_group(group)) }
        @if result.bom_groups.is_empty() {
            div class="text-center py-8 text-muted" { "该产品无关联BOM" }
        }
    }
}
}
 ...
 html! {
    div class="mb-6" {
        div class="flex items-center gap-3 mb-3" {
            span
                class="inline-flex items-center gap-1 px-3 py-0.5 rounded-full text-xs font-semibold bg-accent-bg text-accent"
            { (icon::box_icon("w-3.5 h-3.5")) "BOM" }
            span class="text-base font-semibold text-fg" { (group.bom_name) }
        }
        div class="data-card" {
            div class="overflow-x-auto" {
                table class="data-table" {
                    ...
                    tbody {
                        @for child in &group.children { (bom_child_row(child)) }
                        @if group.children.is_empty() {
                            tr {
                                td colspan="7" class="text-center py-8 text-muted" { "无子件数据" }
                            }
                        }
                    }
                }
...
fn bom_child_row(child: &ChildNodeInventory) -> Markup {
 ...
 html! {
    tr {
        ...
        td {
            @if let Some(ref u) = child.unit { (u) } @else {
        ...
        td class="text-right text-[13px] font-mono tabular-nums" { (format!("{:.1}%", loss_pct)) }
        td {
            @if is_shortage {
                ...
                span class="text-success font-medium font-mono" { "充足" }
            }
        }
    }
}
