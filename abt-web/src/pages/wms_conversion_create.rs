...
fn conversion_create_page(
    ...
    html! {
        div {
            a href=(format!("{}?restore=true", ConversionListPath::PATH)) class="inline-flex items-center gap-2 text-sm text-text-muted hover:text-accent transition-colors duration-150" {
            ...
            div class="flex items-center" {
                ...
                div class="flex items-center gap-2 text-xs text-text-muted" { span class="w-[10px] h-[10px] rounded-full bg-border" {} "已完成" }
            }
            ...
            form hx-post=(ConversionCreatePath::PATH) hx-swap="none" id="conversionForm"
                ...
                div class="flex items-center justify-end gap-3 pt-4 border-t border-border-soft" {
                    a href=(format!("{}?restore=true", ConversionListPath::PATH)) class="inline-flex items-center gap-2 rounded-sm text-sm font-medium cursor-pointer whitespace-nowrap relative bg-white text-fg border border-border hover:bg-surface" { "取消" }
