...
fn item_row_fragment(product: &abt_core::master_data::product::model::Product) -> Markup {
 ...
 td {
 input class="w-full px-2 py-[5px] text-[13px] border border-border rounded-sm bg-white text-fg outline-none focus:border-accent" type="number" step="any" name="bin_id" placeholder="库位ID" {}
 }
 td {
 ...
 td {
 input class="num-input w-full text-right px-2 py-[5px] text-[13px] font-mono tabular-nums border border-border rounded-sm bg-white text-fg outline-none focus:border-accent" type="number" step="any" name="system_qty" placeholder="0" {}
 }
 td {
