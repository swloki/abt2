        ...
        div {
            ...
            form hx-post=(RequisitionCreatePath::PATH) hx-swap="none" id="requisitionForm"
                ...
                div class="action-bar" {
                    a href=(format!("{}?restore=true", RequisitionListPath::PATH)) class="inline-flex items-center gap-2 rounded-sm text-sm font-medium cursor-pointer whitespace-nowrap relative bg-white text-fg border border-border hover:bg-surface" { "取消" }
                ...
                @for p in products {
                    div class="flex items-center justify-between p-3 border-b" {
                        ...
                        button type="button" class="btn inline-flex items-center gap-2 rounded-sm text-sm font-medium cursor-pointer whitespace-nowrap relative-sm bg-accent text-accent-on border-none hover:bg-accent-hover"
                            ...
                            hx-swap="beforeend"
                            _="on 'htmx:afterRequest'[detail.xhr.status < 400] remove .is-open from #product-modal then wait 50ms then call reqRenumber()" {
                            "选择"
                        }
                    }
                }
