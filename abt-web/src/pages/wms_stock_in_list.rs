            ...
            div class="flex gap-3" {
                ...
                @if can_create {
                    a   class="inline-flex items-center gap-2 py-[9px] px-[18px] rounded-sm bg-accent text-accent-on border-none hover:bg-accent-hover text-sm font-medium cursor-pointer transition-all duration-150 shadow-[0_1px_2px_rgba(37,99,235,0.2)]"
                        href=(StockInCreatePath::PATH)
                    { (icon::plus_icon("w-4 h-4")) "新建入库单" }
                }
            }
        }
    ...
    div class="stockin-list-panel" {
           ...
           
        {
                ...
                name="transaction_type"
            {
                ...
                option value="PurchaseReceipt" selected[selected_type == "PurchaseReceipt"] {
                    "采购入库"
                }
                option value="ProductionReceipt" selected[selected_type == "ProductionReceipt"] {
                    "生产入库"
                }
            }
            select
            ...
            input
                class="w-[140px] px-3 py-2 border border-border rounded-sm text-sm bg-white text-fg outline-none cursor-pointer"
                type="date"
                name="date_start"
            ...
            table class="data-table" {
                ...
                tbody {
                    @for item in &result.items {
                        @let (type_label, type_cls) = transaction_type_label(
                            &item.transaction_type,
                        );
                        @let wh_name = wh_names
                            .get(&item.warehouse_id)
                            .map(|s| s.as_str())
                            .unwrap_or("—");
                        ...
                        tr {
                            ...
                            { (item.doc_number.as_deref().unwrap_or("—")) }
                            td {
                                span
                                    class=({
                                        format!(
                            ...
                            td class="font-mono tabular-nums text-fg-2 text-xs" {
                                @if let Some(ref sn) = item.source_doc_number { (sn) } @else {
                            ...
                            td {
                                span
                                    class="inline-flex items-center gap-1 rounded-full text-xs font-medium px-2 py-0.5 bg-success-bg text-success"
                                { "已入库" }
                            }
                            ...
                            td {
                                a   href=(format!("/admin/wms/stock-in/{}", item.id))
                    ...
                    @if result.items.is_empty() {
                        tr {
                            td colspan="12" class="text-center text-muted py-8" { "暂无入库记录" }
                        }
                    }
                }
            }
