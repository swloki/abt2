            ...
            div class="flex gap-3" {
                ...
                @if can_create {
                    a   class="inline-flex items-center gap-2 py-[9px] px-[18px] rounded-sm bg-accent text-accent-on border-none hover:bg-accent-hover text-sm font-medium cursor-pointer transition-all duration-150 shadow-[0_1px_2px_rgba(37,99,235,0.2)]"
                        href=(StockOutCreatePath::PATH)
                    { (icon::upload_icon("w-4 h-4")) "新建出库单" }
                }
            }
        }
        // ── Tabs + Filter + Table (HTMX panel) ──
        ({
            stock_out_table_fragment(
                result,
                operator_names,
                wh_names,
                warehouses,
            ...
            table class="data-table" {
                ...
                tbody {
                    @for item in &result.items {
                        @let (type_label, type_cls) = out_type_label(
                        ...
                        tr {
                            ...
                            td {
                                a   class="font-mono tabular-nums text-accent hover:underline"
                                    href=(detail_url)
                            ...
                            td class="font-mono tabular-nums text-xs text-fg-2" {
                                (format!("{}-{}", item.source_type, item.source_id))
                            }
                            ...
                            td {
                                a href=(detail_url) class="text-accent text-xs hover:underline" {
                                    "详情"
                                }
                            }
                        }
                    ...
                    @if result.items.is_empty() {
                        tr {
                            td colspan="13" class="text-center text-muted py-8" { "暂无出库记录" }
                        }
                    }
                }
            }
