...
fn transfer_create_page(
 ...
 div id="transfer-product-modal" class="fixed inset-0 z-[1000] grid place-items-center bg-[rgba(15,23,42,0.45)] backdrop-blur-sm opacity-0 pointer-events-none transition-opacity duration-200 [&.is-open]:opacity-100 [&.is-open]:pointer-events-auto"
...
fn product_list_fragment(products: &[abt_core::master_data::product::model::Product]) -> Markup {
 ...
 hx-swap="beforeend"
 _="on 'htmx:afterRequest'[detail.xhr.status < 400] remove .is-open from #transfer-product-modal then wait 50ms then call transferRenumber()" {
 "选择"
 }
 }
 }
