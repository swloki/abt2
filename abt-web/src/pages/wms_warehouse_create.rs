 ...
 html! {
    div {
        ...
        div class="flex items-center justify-between mb-6" {
            ...
            { (icon::arrow_left_icon("w-4 h-4")) "返回仓库管理列表" }
            h1 class="text-xl font-bold text-fg tracking-tight" { (title) }
        }

        form id="warehouse-form" hx-post=(form_action) hx-swap="none" class="space-y-5" {
            // ── Section: 基本信息 ──
            div class="data-card mb-4" {
                div class="flex items-center gap-2 text-sm font-semibold text-fg mb-4 pb-2 border-b border-border-soft"
                { (icon::building_icon("w-4 h-4")) " 基本信息" }
                div class="grid grid-cols-2 gap-4 gap-x-6 mb-6" {
                    ...
                    div class="form-field" {
                        ...
                        input type="text" name="name" required placeholder="请输入仓库名称" value=(name_val);
                    }
                    div class="form-field" {
                        label {
                            "仓库类型 "
                        ...
                        select name="warehouse_type" required id="warehouse-type-select" {
                    ...
                    div class="form-field" id="address-field" {
                        label { "地址" }
                        input type="text" name="address" placeholder="请输入仓库地址" value=(address_val);
                    }
            ...
            div class="sticky bottom-0 flex items-center justify-end gap-3 px-6 py-4 bg-bg border-t border-border-soft"
            {
                ...
                { (icon::check_circle_icon("w-4 h-4")) "保存仓库" }
            }
        }
