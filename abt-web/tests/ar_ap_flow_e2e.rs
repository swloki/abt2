...
//! - 通过 ArApService::post_entry 立 AR/AP 台账（不经发票实体，业务单据直接立账）
//! - settle 核销（CashJournal 收/付款 ↔ ShippingRequest/ArrivalNotice 应收应付）
//! - amount_applied 累加验证（sqlx 直查 ArApLedger 实体验证，不再直访 repo）
...
use abt_core::fms::ar_ap::model::{ArApLedger, PostLedgerEntryReq, SettleReq};
...
async fn k1_ar_ledger_settle_cycle() {
    ...
    let ar_id = svc
        .post_entry(
            ...
            PostLedgerEntryReq {
                ...
                source_id: ar_src,
                source_doc_no: "TEST-SO-001".into(),
    ...
    svc.post_entry(
        ...
        PostLedgerEntryReq {
            ...
            source_id: pay_src,
            source_doc_no: "TEST-PAY-001".into(),
    ...
    let ar: ArApLedger = sqlx::query_as("SELECT * FROM ar_ap_ledger WHERE id = $1")
        .bind(ar_id)
        .fetch_one(&mut *conn)
        .await
        .unwrap();
...
async fn k2_ap_ledger_settle_cycle() {
    ...
    let ap_id = svc
        .post_entry(
            ...
            PostLedgerEntryReq {
                ...
                source_id: ap_src,
                source_doc_no: "TEST-AN-001".into(),
                direction: LedgerDirection::Credit,
    ...
    svc.post_entry(
        ...
        PostLedgerEntryReq {
            ...
            source_type: DocumentType::CashJournal,
            source_id: pay_src,
            source_doc_no: "TEST-PAY-002".into(),
            direction: LedgerDirection::Debit,
            amount: Decimal::from(400),
            currency: "CNY".into(),
            exchange_rate: Decimal::ONE,
    ...
    // 验证：AP 台账全额核销
    let ap: ArApLedger = sqlx::query_as("SELECT * FROM ar_ap_ledger WHERE id = $1")
        .bind(ap_id)
        .fetch_one(&mut *conn)
        .await
        .unwrap();
