...
async fn find_wo_id(app: &TestApp, product: i64) -> i64 {
    ...
    let mut conn = app.state.pool.acquire().await.unwrap();
    let res = svc
        .list(&ctx, &mut conn, WorkOrderFilter { status: None, product_id: Some(product), keyword: None, date_from: None, date_to: None, product_code: None, work_center_id: None }, 1, 10)
        .await
        .unwrap();
