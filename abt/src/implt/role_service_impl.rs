...
impl RoleService for RoleServiceImpl {
    async fn create(&self, operator_id: i64, req: CreateRoleRequest, executor: Executor<'_>) -> Result<i64> {
    ...
    async fn update(&self, operator_id: i64, role_id: i64, req: UpdateRoleRequest, executor: Executor<'_>) -> Result<()> {
    ...
    async fn delete(&self, operator_id: i64, role_id: i64, executor: Executor<'_>) -> Result<()> {
        let old_role = RoleRepo::find_by_id_with_executor(executor, role_id).await?.ok_or_else(|| anyhow!("Role not found"))?;
    ...
    async fn assign_permissions(&self, operator_id: i64, role_id: i64, permission_ids: Vec<i64>, executor: Executor<'_>) -> Result<()> {
        RoleRepo::assign_permissions(executor, role_id, &permission_ids).await?;
        self.log_audit(executor, operator_id, "role", role_id, "assign_permissions", None, Some(serde_json::to_value(&permission_ids)?)).await?;
        Ok(())
    }

    async fn remove_permissions(&self, operator_id: i64, role_id: i64, permission_ids: Vec<i64>, executor: Executor<'_>) -> Result<()> {
        RoleRepo::remove_permissions(executor, role_id, &permission_ids).await?;
        self.log_audit(executor, operator_id, "role", role_id, "remove_permissions", Some(serde_json::to_value(&permission_ids)?), None).await?;
        Ok(())
    }
}
