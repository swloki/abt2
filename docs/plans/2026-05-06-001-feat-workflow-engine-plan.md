...
- R6. auto_task：每个节点独立事务执行，ActionRegistry trait 注册，失败 suspended + 手动重试。支持 `input_mapping` / `output_mapping` 声明式数据映射，action 接收类型化参数而非整个 instance
- R6b. NodeResult：统一节点执行结果抽象（Completed / Waiting / Failed），引擎按结果类型统一调度
...
- R12. frozen_graph 模式版本化（Improvement 8）：`graph_version` 字段随快照传播，引擎按版本分发反序列化
- R13. ActionRegistry 启动时校验（Improvement 9）：fail-closed 验证 active 模板引用的 action 已注册
- R14. 状态列 CHECK 约束（Improvement 10）：instances/tasks/templates 的 status 列数据库级约束
...
- **JSONB 类型演进需版本化**：products.meta 等无版本策略导致类型不安全。frozen_graph 必须加 `graph_version`（见 R12）
- **CHECK 约束保护闭合词汇**：bom.status 已用 CHECK 约束先例。工作流状态枚举同样需要数据库级保护（见 R14）
...
- **声明式数据映射**：auto_task 节点通过 `input_mapping` / `output_mapping` 配置数据流，action 接收 `HashMap<String, Value>` 而非整个 instance，实现任务与流程上下文解耦
- **统一节点执行结果**：所有节点返回 `NodeResult`（Completed / Waiting / Failed），引擎按结果类型统一分发，不感知具体节点内部逻辑
- **frozen_graph 模式版本化**：`graph_version: u32` 随 frozen_graph 传播，引擎按版本分发反序列化路径，防止代码演进破坏运行中实例
- **ActionRegistry 启动时校验**：fail-closed 验证所有 active 模板引用的 action 已注册，延续权限缓存 `.expect()` 模式
- **状态列 CHECK 约束**：沿袭 `bom.status` 先例，数据库级保护闭合状态词汇
...
abt/src/models/workflow.rs                     ← 数据模型 + Graph 类型 + Condition AST + NodeResult
...
abt/src/implt/workflow_engine.rs               ← 核心引擎（start_instance, advance_instance, 数据映射）
abt/src/implt/workflow_actions.rs              ← AutoAction trait + ActionRegistry + 表达式映射引擎
...
- status 列添加 CHECK 约束（见 spec Improvement 10）：`workflow_instances.status`、`workflow_tasks.status`、`workflow_templates.status`
...
- 幂等性: 重复运行不报错
- CHECK 约束验证: INSERT workflow_instances with status='invalid' → 约束违反错误
- CHECK 约束验证: INSERT workflow_tasks with status='pending' → 成功
- CHECK 约束验证: UPDATE workflow_templates SET status='active' WHERE status='draft' → 成功
- CHECK 约束验证: UPDATE workflow_instances SET status='runing' → 约束违反（拼写错误被拦截）

**Verification:** `cargo build` 通过，sqlx 编译时查询验证通过，CHECK 约束可拦截非法状态值
...
- graph JSONB 映射为 Rust struct（`WorkflowGraph { graph_version: u32, nodes: Vec<WorkflowNode>, edges: Vec<WorkflowEdge> }`），用 serde 序列化。`graph_version` 随 frozen_graph 快照传播，引擎按版本分发反序列化（见 spec Improvement 8）
...
- `graph_version` 版本分发策略（见 spec Improvement 8）：V1 仅支持 `graph_version = 1`。反序列化 frozen_graph 时检查版本号，不匹配则返回错误（而非 panic）。未来版本可增加版本分发逻辑和迁移函数。模板发布时自动设置当前引擎版本号。
...
- Happy path: WorkflowGraph 包含 `graph_version: 1` 字段，反序列化成功
- Edge case: 空 graph 反序列化
- Edge case: Condition AST 嵌套深度 3 层
- Edge case: frozen_graph 的 `graph_version` 不匹配当前引擎版本 → 返回明确错误（非 panic）
- Edge case: frozen_graph 缺少 `graph_version` 字段 → 返回明确错误

**Verification:** `cargo build` 通过，proto 编译成功，model 可序列化/反序列化，graph_version 校验逻辑测试通过
...
- start_instance 实现：
  1. 按实体类型查找活动模板
  2. 冻结图快照（深拷贝 JSONB，**保留 `graph_version` 字段**——见 R12/Improvement 8）
...
- Happy path: start_instance 创建的 frozen_graph 保留 graph_version 字段
- Edge case: CHECK 约束阻止非法 instance 状态转换（如 completed → running）

**Verification:** `cargo clippy` 通过，线性流程（start → approval → end）端到端可运行，frozen_graph 含 graph_version
...
**Approach:**
- AutoAction trait：`async fn execute(&self, tx: &mut Transaction, inputs: HashMap<String, Value>) -> Result<ActionOutput>`
- ActionOutput：`data: HashMap<String, serde_json::Value>`（action 输出字段，引擎根据 output_mapping 写入上下文）
- ActionRegistry：`HashMap<String, Arc<dyn AutoAction>>`，提供 register/get/is_registered
- ActionRegistry 启动时校验（见 spec Improvement 9）：所有 action 注册完成后，查询 `SELECT DISTINCT` 所有 active 模板的 auto_task action 名称，验证 is_registered。任一未注册 → panic（fail-closed）。在 `init_context_with_pool()` 完成后、服务启动前执行
- 表达式映射引擎：解析 `${变量名}` 语法，支持点号路径（如 `${entity_snapshot.product_id}`），从 WorkflowContext 提取值
- execute_auto_task 方法：
  1. 开新事务
  2. SET LOCAL statement_timeout（从 config.timeout_seconds 取，默认 5s）
  3. **输入映射**：解析节点的 input_mapping，从 instance.context 提取值构造 inputs
  4. 调用 action.execute(&mut tx, inputs)
  5. 成功 → commit + **输出映射**：根据节点的 output_mapping 将 ActionOutput.data 写入 context.variables + 记录 history → 评估出边继续推进
  6. 失败 → rollback → 独立事务 suspend instance → 返回错误
...
- Edge case: auto_task 输出通过 output_mapping 写入上下文 → 后续条件可引用
- Happy path: input_mapping 从 entity_snapshot 提取参数 → action 接收类型化输入
- Edge case: 同一 action 在不同流程中通过不同 input_mapping 复用
- Error path: ActionRegistry 启动校验——active 模板引用未注册 action → panic（fail-closed）
- Happy path: ActionRegistry 启动校验——所有 active 模板的 action 均已注册 → 校验通过
- Edge case: ActionRegistry 启动校验——无 active 模板 → 校验通过（无需检查）
- Edge case: ActionRegistry 启动校验——active 模板无 auto_task 节点 → 校验通过

**Verification:** auto_task 执行、失败 suspended、重试恢复 全流程可运行，ActionRegistry 启动校验 fail-closed 行为验证
...
| frozen_graph schema 演进 | `graph_version` 版本化（R12）保护运行中实例。V1 仅支持版本 1，不匹配返回错误而非 panic |
| Action 注册遗漏 | 启动时校验（R13）fail-closed：active 模板引用未注册 action → 拒绝启动 |
| 状态枚举扩展 | CHECK 约束（R14）需 ALTER TABLE 追加新值，但状态变化频率极低 |
