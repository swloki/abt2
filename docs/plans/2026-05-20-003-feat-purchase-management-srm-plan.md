...
abt/migrations/
  045_create_document_sequences.sql
  046_create_supplier_tables.sql
  047_create_purchase_tables.sql
```
...
- Create: `abt/migrations/046_create_supplier_tables.sql`
- Create: `abt/migrations/047_create_purchase_tables.sql`

**Approach:**
- 045: `document_sequences` 表 + PO/PS/PP 三行种子数据（`INSERT ... ON CONFLICT DO NOTHING`）
- 046: `suppliers` + `supplier_contacts` + `supplier_bank_accounts` + `supplier_prices` 四张表 + 索引
- 047: `purchase_orders` + `purchase_order_items` + `purchase_statements` + `purchase_statement_items` + `purchase_invoices` + `purchase_payments` 六张表 + 索引
...
- 编号格式 `PO-2026-05-00001`，月度重置时检查 `updated_at` 是否跨月
...
**Test scenarios:**
- Happy path: `next_number("PO")` 返回 `PO-2026-05-00001`，连续调用返回 `00002`、`00003`
