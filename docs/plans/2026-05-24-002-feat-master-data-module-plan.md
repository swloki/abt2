...
- `Supplier`：id, code, name, short_name, category, status, currency(VARCHAR(3), ISO 4217 三字母编码，默认 CNY，支持 CNY/JPY/USD/AUD/EUR), tax_number, lead_time_days, payment_terms, remark, operator_id, created_at, updated_at, deleted_at
...
- `CreateSupplierReq`：包含 `currency: Option<String>`（默认 CNY）
