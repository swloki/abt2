...
**Verification:** 在 Shell 中 `source config/env.sh && echo $ABT_URL` 返回 `https://localhost:8000`。
...
**Verification:** 运行 `source lib/login.sh && abt_login test_session admin chenxi0514` 成功，浏览器在仪表盘页面。
