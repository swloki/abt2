...
**Files:**
- Create: `migrations/010_create_permission_tables.sql`

**Goal:** 创建 RBAC 所需的数据库表和预置数据

---

## Step 1: 编写迁移 SQL

创建文件 `migrations/010_create_permission_tables.sql`：
...
-- 预期：3 条记录 (super_admin, admin, user)

-- 验证操作
SELECT action_code, action_name FROM actions ORDER BY sort_order;
-- 预期：3 条记录 (read, write, delete)

-- 验证资源
SELECT resource_code, resource_name, group_name FROM resources ORDER BY sort_order;
-- 预期：12 条记录

-- 验证权限
SELECT COUNT(*) FROM permissions;
-- 预期：36 条记录 (12 资源 × 3 操作)
```
...
```bash
git add migrations/010_create_permission_tables.sql
git commit -m "feat(rbac): add permission tables migration

- Create users, roles, resources, actions, permissions tables
- Add user_roles and role_permissions relation tables
- Add permission_audit_logs for audit trail
- Insert preset data: 3 roles, 3 actions, 12 resources, 36 permissions
