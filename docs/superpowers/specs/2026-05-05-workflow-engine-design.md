...
| event_type | VARCHAR | node_enter / edge_triggered / condition_evaluated / task_completed / timeout_action / hook_executed / back_to_previous / suspended |
...
7. 计算 `graph_checksum`（SHA-256，对规范化 JSON 字符串 `canonicalize(graph.nodes) ++ canonicalize(graph.edges)` 计算哈希）用于完整性校验
...
pub async fn advance_instance(task_id: Uuid, action: Action, result: Value) -> Result<()> {
    ...
    // 1. 根据 task_id 查询 instance_id，再锁实例
...
当一个 `approval` 节点的 `multi_approval = "all"` 时，该节点会创建多个并行 task（每个审批人一个）。任一 task 完成时不立即推进，而是检查同一 `node_id` 下是否还有 `pending` task。只有当该节点所有 task 都完成后才评估出边并推进。任一 task 被 rejected 时，根据 `reject_action` 决定是否终止。
...
- Worker 扫描频率：默认 5 分钟（默认值存于环境变量 `WORKER_SCAN_INTERVAL_SECS`），可通过 Admin API 动态调整。优先级：Admin 配置 > 环境变量
...
- 边界测试：驳回后退回、并行节点部分完成、条件分支选择、assignee 解析为零候选人的 fail-closed、fallback 不存在的 suspended 行为、**并行 + multi_approval=all + reject 组合场景**（重点覆盖）
