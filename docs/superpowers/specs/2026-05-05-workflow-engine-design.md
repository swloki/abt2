...
- `rejected` — 被审批人驳回且 reject_action = terminate
...
4. Condition AST 字段白名单 + 类型校验（创建时初步校验）
...
**`back_to_previous`：**
1. 标记当前 task 为 rejected
2. 沿 `frozen_graph` 反向查找当前节点的入边，确定前驱节点
3. 在前驱节点创建新 task，`prev_task_id` 指向当前 rejected task
4. 新 task 的 `result` 包含 `{"rejection_context": {"original_task_id": "...", "reason": "..."}}`
5. V1 只支持退回到直接前驱节点，不支持指定退回目标或跳级退回
...
- Worker 扫描频率：默认 5 分钟（默认值存于环境变量 `WORKER_SCAN_INTERVAL_SECS`），可通过 Admin API 动态调整
