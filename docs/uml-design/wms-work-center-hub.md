...
> 状态：**已实现**（单端点 + hx-select-oob，2026-06 落地，见 §5）；**2026-06 tab 化 + 数据库分页**：锚点条 + tab 主体（5 tab + 分页 + 过滤），`WorkCenterRepo` 跨域聚合 SQL 真正分页（只查当前页 20 条），见 §2.2 / §4.4 / §5；**2026-07 出库流合并**：删「待拣货」并入「待出库」（6→5 tab），消除重复计数 + 对齐主流 ERP，见 §3 / §9.6
...
- **5 环节 tab 主体**（替代原卡片网格 / disclosure）：切 tab 看该环节待办队列（分页 + 过滤 + 紧急度排序），**不是跳转**
