...
> 状态：**设计骨架，待评审**（接口先行 → 评审 → 实现）
> 现行实现：`abt-web/src/pages/wms_work_center.rs`（7 卡片网格 + 跳转 + 假紧急数据）；`WorkCenterService::summary`
...
- **Phase C（就地操作）**：拣货 / 收货 / 确认发出 drawer 接各域 service（与 Doc Hub 共享组件）。
- **Phase D（配置化）**：临期 N 天 / 拣货超时阈值进 `wms/settings`。

每阶段独立 PR，远程 `weichen`，feature 分支（`feat/wms-work-center-hub-*`）。
