...
# 领料前记录库存
log_step "0. 记录领料前库存"
BEFORE_RM001=$(psql "$DB_URL" -t -A -c "SELECT COALESCE(SUM(quantity),0) FROM stock_ledger WHERE product_id=$PRODUCT_RM001 AND deleted_at IS NULL" 2>/dev/null || echo "0")
BEFORE_RM002=$(psql "$DB_URL" -t -A -c "SELECT COALESCE(SUM(quantity),0) FROM stock_ledger WHERE product_id=$PRODUCT_RM002 AND deleted_at IS NULL" 2>/dev/null || echo "0")
BEFORE_RM003=$(psql "$DB_URL" -t -A -c "SELECT COALESCE(SUM(quantity),0) FROM stock_ledger WHERE product_id=$PRODUCT_RM003 AND deleted_at IS NULL" 2>/dev/null || echo "0")
...
abt_assert_url_contains "$AGENT_W1_SESSION" "/admin/wms/requisitions/create" "领料单创建页"
...
abt_click_by_text "$AGENT_W1_SESSION" "提交领料单"
sleep 2
...
# 检查库存是否减少
AFTER_RM001=$(psql "$DB_URL" -t -A -c "SELECT COALESCE(SUM(quantity),0) FROM stock_ledger WHERE product_id=$PRODUCT_RM001 AND deleted_at IS NULL" 2>/dev/null || echo "0")
AFTER_RM002=$(psql "$DB_URL" -t -A -c "SELECT COALESCE(SUM(quantity),0) FROM stock_ledger WHERE product_id=$PRODUCT_RM002 AND deleted_at IS NULL" 2>/dev/null || echo "0")
AFTER_RM003=$(psql "$DB_URL" -t -A -c "SELECT COALESCE(SUM(quantity),0) FROM stock_ledger WHERE product_id=$PRODUCT_RM003 AND deleted_at IS NULL" 2>/dev/null || echo "0")
log_info "领料后: PRD-RM-001=$AFTER_RM001, PRD-RM-002=$AFTER_RM002, PRD-RM-003=$AFTER_RM003"

# 验证库存减少（至少有出入库记录）
abt_assert_db \
    "SELECT 1 FROM inventory_transactions WHERE product_id = $PRODUCT_RM001 AND transaction_type = 'MATERIAL_ISSUE' AND deleted_at IS NULL" \
    "数据库: PRD-RM-001 有领料出库记录" || \
    abt_assert_db \
        "SELECT 1 FROM inventory_transactions WHERE product_id = $PRODUCT_RM001 AND deleted_at IS NULL" \
        "数据库: PRD-RM-001 有库存事务记录" || true
