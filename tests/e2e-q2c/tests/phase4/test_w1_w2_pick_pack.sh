...
# 检查成品库存
FG_STOCK=$(psql "$DB_URL" -t -A -c "SELECT COALESCE(SUM(quantity),0) FROM stock_ledger WHERE product_id=$PRODUCT_FG_ID AND warehouse_id=$WH_FG_ID AND deleted_at IS NULL" 2>/dev/null || echo "0")
...
abt_assert_url_contains "$AGENT_W1_SESSION" "/admin/shipping/create" "发货创建页"
...
# 验证联系人信息出现
abt_assert_visible "$AGENT_W1_SESSION" "#customerInfoBar" "客户信息条" || \
    abt_assert_visible "$AGENT_W1_SESSION" "#shipping-contact" "联系人" || true
...
if [[ -n "$SHIP_ID" ]]; then
    ...
    # 点击"确认发货"按钮 (hx-post to /admin/shipping/{id}/confirm)
...
if [[ -n "$SHIP_ID" ]]; then
    ...
    # 点击"开始拣货"按钮 (hx-post to /admin/shipping/{id}/pick)
